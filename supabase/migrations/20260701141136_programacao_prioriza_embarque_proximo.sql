-- Etapa B: prioriza colaboradores próximos ao ponto de embarque antes do score
-- de contrato/auditoria. Acima de 400 km o candidato ainda pode aparecer, mas
-- fica depois das opções próximas e a UI sugere hospedagem.
...
top8 as (
  select
    s.*,
    row_number() over (
      partition by s.os_id
      order by
        case when s.km is not null and s.km <= 400 then 0 else 1 end,
        s.km asc nulls last,
        s.score desc
    ) as rn
  from scored s
...
order by
  os_id,
  case when km is not null and km <= 400 then 0 else 1 end,
  km asc nulls last,
  score desc
;
$$;
