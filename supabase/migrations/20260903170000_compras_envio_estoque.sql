-- "Enviar Estoque": em Solicitações, itens já disponíveis no almoxarifado podem
-- ser atendidos direto do compras_estoque, sem passar por cotação/compra. A
-- baixa de quantidade é automática (compras_estoque_materiais/movimentacoes);
-- o item sai de Solicitações marcado com o novo status abaixo.
alter table public.compras_itens add column envio_tipo text check (envio_tipo in ('correios','retirada'));
alter table public.compras_itens add column estoque_movimentacao_id uuid references public.compras_estoque_movimentacoes(id);
...
alter table public.compras_itens add constraint compras_itens_status_check
  check (status in (
    ...
    'concluido','cancelado','recusado_financeiro','enviado_estoque'
  ));
...
alter table public.compras_solicitacoes add constraint compras_solicitacoes_status_check
  check (status in (
    'pendente','em_cotacao','em_analise','pendente_pagamento',
    'aguardando_termo','aguardando_nf','comprado','recusado',
    'concluido','cancelado','aberto','recusado_financeiro','enviado_estoque'
  ));
...
begin
  select
    case
      ...
      when bool_and(status = 'enviado_estoque') then 'enviado_estoque'
