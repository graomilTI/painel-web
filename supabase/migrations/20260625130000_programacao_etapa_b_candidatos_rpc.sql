-- Etapa B da Programação: move o ranking de candidatos (Contrato/Distância/
-- Auditoria) para o banco. Antes, o navegador baixava todos os colaboradores
-- da supervisão (até ~3500) e recalculava haversine + sort para cada OS em
-- ATENDER (até 300), travando a tela em supervisões grandes. Agora o cálculo
-- roda em SQL sobre colaborador_cruzamento (já pré-computado via pg_cron) e
-- só os top 8 candidatos por OS voltam para o front.
...
-- colaborador_cruzamento tem colaborador_id (uuid) como PK, não cpf — pode
-- haver mais de um cadastro com o mesmo CPF, então deduplica antes do join
-- (sem isso, cada candidato saía repetido por OS).
cz_best as (
  select distinct on (cpf) cpf, tipo_contrato, latitude, longitude, auditorias_180d_qtd, auditorias_180d_peso
...
scored as (
  select
    ...
    f.km, f.auditorias_qtd, f.auditorias_peso,
...
select os_id, colaborador_id, nome, cargo, coordenacao, supervisao, tipo_contrato,
       km, auditorias_qtd, auditorias_peso, score, score_contrato, score_distancia, score_auditoria
from top8
where rn <= 8
