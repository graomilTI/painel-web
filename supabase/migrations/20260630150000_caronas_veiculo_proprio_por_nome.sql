-- A relação de veículo próprio é cadastrada por NOME (tela Conferência ·
-- Deslocamento). Como nem todo colaborador casa pela chave (cpf-norm/nome),
-- a RPC passa a marcar veiculo_proprio por colaborador_id OU por nome
-- normalizado (unaccent), via EXISTS (sem duplicar linhas).
