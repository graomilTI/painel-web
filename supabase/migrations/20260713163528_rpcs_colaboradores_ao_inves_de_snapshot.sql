-- Migra as 2 RPCs vivas que ainda liam colaborador_snapshot (base legada, parada
-- em 2026-06-18) para colaboradores_atuais (view sobre a tabela colaboradores,
-- sincronizada continuamente). Mantém a mesma lógica de elegibilidade
-- (ativo/desligamento/situacao) e a mesma assinatura/retorno; só remove o
-- CTE "ref = max(data_referencia)" pois colaboradores não tem histórico —
-- colaboradores_atuais já representa sempre o estado corrente.
--
-- programacao_cargo_bloqueado_embarque (das migrations 20260630163500/170500)
-- não existe mais no banco — era usada só para o DML pontual de limpeza
-- daquelas migrations, não ficou como função persistente. Nada a migrar ali.
