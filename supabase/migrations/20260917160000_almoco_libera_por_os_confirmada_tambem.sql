-- Bug ao vivo em 17/09/2026: 11 colaboradores da MATO GROSSO MT3 - Querencia
-- (e outros 18 em outras supervisões, ex.: LUCAS MATHEUS DE ARAUJO GOMES,
-- CPF 05705272103) tinham O.S. confirmada em programacao_equipe e
-- programacao_alimentacao.almoco=true, mas o Almoço nunca chegava no Caixa
-- Operacional do GRM (Café/Janta/Táxi/Uber/Serviços Terceirizados saíam
-- normalmente). Causa: o guard de ALMOCO em grm_filtrar_regras_programacao
-- só aceitava v_na_programacao (linha em programacao_colaboradores, o
-- roster genérico da supervisão) e ignorava v_em_os (confirmado numa O.S.
-- via programacao_equipe), que já é calculado ao lado mas só era usado
-- pelas regras de vínculo (Intermitente/Terceirizado). Colaborador
-- confirmado em O.S. real pode nunca ganhar uma linha correspondente em
-- programacao_colaboradores (tabelas alimentadas por fluxos distintos), daí
-- o guard bloqueava o Almoço silenciosamente. Alinha ao mesmo critério que
-- adm-conferencia.js já usa pra "vínculo real": confirmado em O.S. OU
-- presente no roster.
...
begin
  ...
  where case
    when upper(unaccent(trim(coalesce(
      ...
      then (v_na_programacao or v_em_os) and v_almoco_programado
