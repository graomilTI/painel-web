-- Etapa B: troca o score "Contrato" (Efetivo=1/Intermitente=0.6/Diarista=0.3,
-- um proxy grosseiro) por um cálculo real de custo de deslocamento:
--   - Efetivo: salário é pago no fim do mês independente do embarque, então
--     o custo marginal de escalar essa pessoa para esta OS é só o combustível.
--   - Intermitente/Diarista: recebem por dia embarcado, então o custo
--     marginal inclui a diária (colaborador_cruzamento.salario) + combustível.
--   - Combustível: (km ida e volta / 10 km por litro) * R$ 7,00/litro.
-- Quem não tem coordenada continua sem score de custo (igual já era feito
-- para distância/auditoria), não é tratado como "grátis".
  ...
  select
    ...
    case
      when p.km is null then null
        ...
        -- mão de obra: Efetivo = 0 (custo fixo já pago no mês, não varia
        -- com o embarque); Intermitente/Diarista = diária (salario)
        (case when upper(coalesce(p.tipo_contrato, '')) like '%EFETIVO%' then 0 else coalesce(p.salario, 0) end)
        -- combustível ida e volta: (km*2 / 10km por litro) * R$7,00/litro
        + (p.km * 2 / 10.0) * 7.0
    end as custo_total
  from pares p
),
