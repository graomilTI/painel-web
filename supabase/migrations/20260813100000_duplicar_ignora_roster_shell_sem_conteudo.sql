...
comment on function public.duplicar_programacao_dia(uuid, date[]) is
  'Duplica atomicamente uma programação e seus vínculos para até cinco datas; estadias HOTEL sempre geram solicitação (nunca são puladas), separadas por reserva-alvo pra nunca misturar Estender com Reservar no mesmo card; destinos com equipe/estadia/alimentação/deslocamento/despesa/frota reais são preservados e ignorados — um roster shell (só programacao_colaboradores, criado automaticamente pelo "Carregar") não conta como conteúdo e é preenchido normalmente.';
