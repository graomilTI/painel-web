-- Hotfix idempotente do fluxo de decisão da Abertura de O.S.
-- Reaplica a migration original que não chegou ao banco de produção e força
-- o PostgREST a recarregar o cache de schema.
...
drop policy if exists grm_abertura_os_execucoes_select_authenticated
