-- Com o backlog fechado (migration anterior) e nenhum código atual escrevendo
-- 'aguardando_gestor' (a tela que gerava esse status foi removida no commit
-- 226cddae, 31/08/2026), removemos o valor da lista de status válidos.

alter table public.compras_itens
  drop constraint if exists compras_itens_status_check;
alter table public.compras_itens
...
alter table public.compras_solicitacoes
  drop constraint if exists compras_solicitacoes_status_check;
alter table public.compras_solicitacoes
