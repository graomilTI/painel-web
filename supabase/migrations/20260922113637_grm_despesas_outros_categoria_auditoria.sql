-- Bug relatado em 22/09/2026: colaborador (Jean Pablo Souza Silva,
-- Coordenador MT2) com "Serviço de auditoria" digitado como despesa OUTROS
-- no fluxo Disponível nunca gerava regra em Regras de Caixa Operacional no
-- GRM, porque "auditoria" não estava em nenhuma das 3 camadas que reconhecem
-- categoria de OUTROS: client (AGENTE_OUTROS_CATEGORIAS em
-- adm-conferencia.js), edge function (configKeyExtra em
-- grm-liberacao-despesas-publicar/index.ts) e esta função SQL. A despesa
-- ficava pendência manual pra sempre (comportamento correto pra categoria
-- desconhecida, mas "auditoria" deveria ser reconhecida). Mesma trava das
-- demais categorias de OUTROS: AUTO sempre falso, conferência/aprovação
-- manual no GRM.
...
insert into public.grm_despesas_tipos_config (
  ...
  (
    'EXTRA_AUDITORIA', 'EXTRA', 'Serviços de Auditoria', 0, true, false,
    true, 1, true,
    'OUTROS com descrição inequívoca de serviço de auditoria. Apenas abre Serviços de Auditoria no Caixa Operacional; AUTO sempre falso e conferência/aprovação manual.'
  )
  ...
  with extras_base as (
    ...
    select
      ...
      array_remove(array[
        ...
        case when e.texto ~ '(^| )AUDITORIA(S)?( |$)'
             then 'EXTRA_AUDITORIA' end
      ], null) as chaves
    from extras_base e
