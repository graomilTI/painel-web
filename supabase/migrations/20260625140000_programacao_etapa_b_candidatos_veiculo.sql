-- Etapa B: inclui veiculo_id/veiculo_placa no retorno da RPC de candidatos,
-- para o front marcar quem já é motorista de um veículo cadastrado
-- (badge "Logística" e capacidade de até 4 classificadores por veículo).
...
scored as (
  select
    ...
    f.km, f.auditorias_qtd, f.auditorias_peso, f.veiculo_id, f.veiculo_placa,
...
select os_id, colaborador_id, nome, cargo, coordenacao, supervisao, tipo_contrato,
       km, auditorias_qtd, auditorias_peso, veiculo_id, veiculo_placa,
