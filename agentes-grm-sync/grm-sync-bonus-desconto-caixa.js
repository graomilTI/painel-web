...
/*
 * GRM Server - Lançamento do Adiantamento de desconto de auditoria no Caixa do colaborador.
 *
 * Fila: bonus_desconto_caixa_lancamentos (gerada por bonus_substituir_auditoria ao
...
const SCREENSHOT_DIR = process.env.GRM_BONUS_DESCONTO_CAIXA_SCREENSHOT_DIR
  || '/home/grao100/painel-scripts/grm-sync/logs/bonus-desconto-caixa';
...
async function recoverStaleProcessing() {
  ...
  const { error } = await supabase
    .from('bonus_desconto_caixa_lancamentos')
...
async function main() {
  ...
  if (!jobs.length) {
    log('INFO', 'Nenhum Adiantamento de desconto de auditoria pendente para lançar.');
    return;
  }
  ...
  log(errors ? 'WARN' : 'SUCCESS', 'Agente de Adiantamento de desconto de auditoria concluído.', {
...
main().catch((error) => {
  log('ERROR', `Erro fatal no agente de Adiantamento de desconto de auditoria: ${error.message}`, { stack: error.stack });
  process.exitCode = 1;
});
