...
PATCH="$REPO_DIR/agentes-grm-sync/patch-bonus-caixa-inativos-v3.js"
PROD_AGENT="$PROD_DIR/grm-sync-bonus-caixa.js"
STAMP="$(date +%Y%m%d-%H%M%S)"
TMP="$(mktemp /tmp/grm-sync-bonus-caixa-inativos-v3.XXXXXX.js)"

cleanup() {
  rm -f "$TMP" "$TMP.bak-inativos-v3" 2>/dev/null || true
}
trap cleanup EXIT
...
if [[ ! -f "$PATCH" ]]; then
  echo "[ERRO] Patch v3 nao encontrado: $PATCH" >&2
  exit 1
fi
...
echo "[INFO] Validando sintaxe do patch v3..."
...
echo "[INFO] Aplicando fallback Ativos -> Inativos v3 em copia temporaria..."
"$NODE_BIN" "$PATCH" "$TMP"

echo "[INFO] Validando copia temporaria adaptada..."
"$NODE_BIN" --check "$TMP"

cp -a "$PROD_AGENT" "$PROD_AGENT.bak-inativos-v3-$STAMP"
echo "[OK] Backup de producao: $PROD_AGENT.bak-inativos-v3-$STAMP"

install -m 750 -o grao100 -g grao100 "$TMP" "$PROD_AGENT"
"$NODE_BIN" --check "$PROD_AGENT"

echo "[OK] Agente de Bonus v3 instalado."
echo "[OK] Fallback: procura CPF em Ativos; se nao encontrar, seleciona Situacao = Inativos explicitamente."
