...
PATCH="$REPO_DIR/agentes-grm-sync/patch-bonus-caixa-inativos-v6.js"
PROD_AGENT="$PROD_DIR/grm-sync-bonus-caixa.js"
STAMP="$(date +%Y%m%d-%H%M%S)"
TMP="$(mktemp /tmp/grm-sync-bonus-caixa-inativos-v6.XXXXXX.js)"

cleanup() {
  rm -f "$TMP" "$TMP.bak-inativos-v6" 2>/dev/null || true
}
trap cleanup EXIT
...
if [[ ! -f "$PATCH" ]]; then
  echo "[ERRO] Patch v6 nao encontrado: $PATCH" >&2
  exit 1
fi
...
echo "[INFO] Validando sintaxe do patch v6..."
...
echo "[INFO] Ajustando Situacao para o valor real do GRM: Não Ativos..."
"$NODE_BIN" "$PATCH" "$TMP"

echo "[INFO] Validando copia temporaria adaptada..."
"$NODE_BIN" --check "$TMP"

cp -a "$PROD_AGENT" "$PROD_AGENT.bak-inativos-v6-$STAMP"
echo "[OK] Backup de producao: $PROD_AGENT.bak-inativos-v6-$STAMP"

install -m 750 -o grao100 -g grao100 "$TMP" "$PROD_AGENT"
"$NODE_BIN" --check "$PROD_AGENT"

echo "[OK] Agente de Bonus v6 instalado."
echo "[OK] Fallback agora seleciona Situacao = Não Ativos, exatamente como o GRM exibe."
