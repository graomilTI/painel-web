...
const v2Marker = Buffer.from('async function setStaffSituation(page, targetSituation) {', 'utf8');

if (original.indexOf(v2Marker) >= 0) {
  console.log(`[OK] Patch v2 de inativos ja aplicado em ${target}`);
  process.exit(0);
}
...
patched = replaceRange(
  patched,
  [
    'async function clearStaffSituationFilter(page) {',
...
async function setStaffSituation(page, targetSituation) {
  ...
  const prepared = await page.evaluate(() => {
    ...
    document.querySelectorAll('[data-grm-bonus-situation-field]')
      .forEach((el) => delete el.dataset.grmBonusSituationField);
    ...
    return { ok: true, text: normalize(root.textContent).slice(0, 160) };
  });
  ...
  try {
    await page.waitForFunction((targetPrefix) => {
      const normalize = (value) => String(value || '')
        .normalize('NFD')
        .replace(/[\\u0300-\\u036f]/g, '')
        .toUpperCase()
      ...
      return [...document.querySelectorAll('.v-list-item,[role="option"],.v-overlay__content *')]
        .filter(visible)
        .some((el) => normalize(el.textContent).startsWith(targetPrefix));
    }, { timeout: 5000 }, target.startsWith('INATIV') ? 'INATIV' : target);
  } catch {
    throw new Error(`Opcoes do filtro Situacao nao abriram para selecionar ${targetSituation}.`);
  }

  const selected = await page.evaluate((targetPrefix) => {
    const normalize = (value) => String(value || '')
      .normalize('NFD')
      .replace(/[\\u0300-\\u036f]/g, '')
      .toUpperCase()
    ...
    const candidates = [...document.querySelectorAll('.v-list-item,[role="option"]')]
      .filter(visible)
      .filter((el) => normalize(el.textContent).startsWith(targetPrefix));

    const option = candidates[0];
    if (!option) return { ok: false, matches: 0 };
    option.click();
    return { ok: true, text: normalize(option.textContent) };
  }, target.startsWith('INATIV') ? 'INATIV' : target);
  ...
  const confirmed = await page.evaluate((targetPrefix) => {
    const normalize = (value) => String(value || '')
      .normalize('NFD')
      .replace(/[\\u0300-\\u036f]/g, '')
      .toUpperCase()
    ...
    return {
      ok: texts.some((text) => text.includes(targetPrefix)),
...
patched = replaceRange(
  ...
  if (!found) {
    ...
    await setStaffSituation(page, 'Inativos');
...
const backup = `${target}.bak-inativos-v2`;
if (!fs.existsSync(backup)) {
  fs.writeFileSync(backup, original);
}

fs.writeFileSync(target, patched);
console.log(`[OK] Patch v2 aplicado em ${target}`);
