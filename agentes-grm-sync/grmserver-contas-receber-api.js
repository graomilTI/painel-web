...
/**
 * Sincroniza Contas a Receber direto pela API do Graint, substituindo o fluxo
 * Puppeteer (grm-sync-contas-receber.js) — mesmo padrão de
 * grmserver-contas-pagar-api.js/grmserver-patrimonios-api.js. O script
 * Puppeteer antigo já fazia `fetch('/api/receiveInvoice/getRecords', ...)` de
 * DENTRO da página; aqui o login também é via API e roda sem navegador.
 */
...
const REPORT_CONFIG = {
  ...
  tableName: 'grm_contas_receber_importacoes',
};
...
async function fetchInvoiceData(token) {
  ...
  const json = await postJson(`${GRM_BASE_URL}receiveInvoice/getRecords`, {
    clnCode: '', clrCode: '', cliCode: '', bilStatus: 'A', staAbreviation: '', bilType: ['C', 'T', 'I', 'S'], bilCode: '',
  }, authHeaders(token));
