...
const REPORT_CONFIG = {
  ...
  url: 'https://www.grmserver.com.br/finance/receiveInvoice',
  apiUrlPattern: 'receiveInvoice/getRecords',
...
async function fetchInvoiceData(page) {
  ...
  const json = await page.evaluate(async () => {
    ...
    const res = await fetch('/api/receiveInvoice/getRecords', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      body: JSON.stringify({ clnCode: '', clrCode: '', cliCode: '', bilStatus: 'A', staAbreviation: '', bilType: ['C', 'T', 'I', 'S'], bilCode: '' })
    });
    return res.json();
  });
...
async function upsertData(data) {
  ...
  // enviado aqui) — insert simples + limpeza do lote anterior por created_at,
  // mesmo padrão do agente API que substitui este script.
