...
### `grm-sync-classificacao-ourosafra.js` (novo, 28/08 — no cron a cada 30min em DRY_RUN, ainda fora do SCRIPT_MAP/fila V2)

Segundo agente de escrita do repo, e o primeiro que grava fora do GRM: casa placas "Aguardando Classificação" no painel Ouro Safra (`app.ourosafra.com.br/app/cdci`) com a classificação já feita no GRM (`report/classification/loads`, filtro Cliente Nacional = OURO SAFRA INDUSTRIA E COMERCIO LTDA), preenche os 3 itens (Impureza/Umidade/Avariados) na Ouro Safra, baixa o laudo em PDF da O.S. correspondente no GRM (`operation/serviceOrder` → Cargas → Imprimir Laudo) e anexa de volta na Ouro Safra. `buscarClassificacaoGRM()` (Cliente Nacional, Placa, período, checkboxes de itens, leitura da tabela) foi re-testada com Puppeteer de verdade em 28/08 contra a placa BDP-1G46/O.S. 90493 e bateu exato — alta confiança. O fluxo do lado Ouro Safra (modal, itens, upload de laudo) e a busca do laudo na O.S. do GRM só foram validados manualmente no Chrome (27/08) — os seletores usam texto/posição estrutural (não IDs fixos, a Ouro Safra é Radzen/Blazor Server) mas **ainda não foram exercitados via Puppeteer**.

Credenciais `OUROSAFRA_USER`/`OUROSAFRA_PASSWORD` já estão no `.env` do servidor. Tabela de auditoria `ouro_safra_classificacao_execucoes` criada no Supabase (migration `20260828130000_ouro_safra_classificacao_execucoes.sql`) — 1 linha por placa processada, alimenta o KPI "Classificação Ouro Safra" em Painel de Conferência (`assets/js/adm-conferencia.js`). Segue o mesmo padrão de segurança do `aplicar-distribuicao-os`: `--dry-run`/`DRY_RUN=true` casa a placa e calcula os valores mas não preenche nem anexa nada; `HEADLESS=false` roda com o Chrome visível. Placa sem correspondência no GRM é pulada silenciosamente (tenta de novo no próximo ciclo).

**Cron** (linha própria com `flock`, fora da fila V2/SCRIPT_MAP — não roda pela `worker/grm-sync-job-worker.js`):
```
*/30 * * * * /usr/bin/flock -n /home/grao100/tmp/grm-classificacao-ourosafra.lock /bin/bash -lc 'cd /home/grao100/painel-scripts/grm-sync && HOME=/home/grao100 TMPDIR=/home/grao100/tmp TMP=/home/grao100/tmp TEMP=/home/grao100/tmp PATH=/opt/node22/bin:/home/grao100/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin DRY_RUN=true /opt/node22/bin/node grm-sync-classificacao-ourosafra.js' >> /home/grao100/painel-scripts/grm-sync/logs/classificacao-ourosafra.log 2>&1
```
Está em `DRY_RUN=true` de propósito — quando aparecer uma placa em "Aguardando Classificação" e o log/KPI mostrarem que o casamento com o GRM saiu certo, tirar o `DRY_RUN=true` da linha do crontab (`crontab -e` no servidor, ou reaplicar a linha acima sem essa variável) pra passar a gravar de verdade.
