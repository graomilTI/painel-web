...
body.mobile-gestor-mode .cmp-request-card .cmp-tabs{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:4px!important;width:100%!important;padding:4px!important;border:1px solid var(--line)!important;border-radius:14px!important;background:rgba(3,20,14,.62)!important}.cmp-request-card .cmp-tab{min-height:42px!important;padding:7px 6px!important;border:1px solid transparent!important;border-radius:10px!important;background:transparent!important;color:var(--muted)!important;font-size:12px!important}.cmp-request-card .cmp-tab.active{background:var(--green-soft)!important;border-color:var(--green-2)!important;color:var(--text)!important}
...
function logistica(){
  ...
  [['osUfEmbarque','Dados do Embarque','emb'],['osUfDestino','Dados do Destino','dst']].forEach(([id,txt,key])=>{const t=q('#'+id,grid)?.closest('label');if(t&&!q('[data-gm-title="'+key+'"]',grid)){const d=document.createElement('div');d.className='gm-log-title';d.dataset.gmTitle=key;d.textContent=txt;t.before(d)}});
