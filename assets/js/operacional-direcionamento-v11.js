...
(function () {
  ...
  const STYLE_ID = 'opv11-direcionamento-style';
  const LEAFLET_CSS = 'leaflet-css-opv11';
  const LEAFLET_JS = 'leaflet-js-opv11';
  ...
  // Cuidado: latitude/longitude usam ponto decimal. Não remover ponto por padrão.
  function num(v) {
    ...
    // BR: 1.234,56 -> 1234.56. Decimal simples: -23.5505 fica intacto.
  ...
  async function sel(table, columns = '*', fn = null, limit = 5000) {
    try {
      ...
      console.warn(`[opv11] ${table}:`, e?.message || e);
      return [];
    }
  }
  ...
  async function load() {
    ...
    let selecionadas = abertas.filter(o => (saldoRemanescente(o) ?? 0) > 0);
    st.fallbackSaldo = false;

    if (!selecionadas.length && abertas.length) {
      st.fallbackSaldo = true;
      selecionadas = abertas;
      const sample = abertas[0] || {};
      console.warn('[opv11] Nenhuma OS passou no filtro de saldo > 0. Fallback aplicado para OS abertas. Colunas da OS:', Object.keys(sample));
    }

    st.os = selecionadas
      .map(o => {
        const p = pontoDaOs(o);
        const saldo = saldoRemanescente(o);
        return p ? { ...o, __pontoKey: p.__key, __saldo: saldo ?? 'não identificado' } : null;
      })
      .filter(Boolean);
  ...
  async function rotaReal(points) {
    ...
    try {
      ...
      console.warn('[opv11] rota real indisponível:', err?.message || err);
  ...
  async function leaflet() {
    ...
    try {
      ...
      console.warn('[opv11] leaflet:', err?.message || err);
      return false;
    }
  }
            ...
            <aside class="opv2-side">
              ...
              <section class="opv2-card">
                <div class="opv2-list">
                  ...
                  `).join('')}
                  ${!base.length && !sem.length ? '<div class="opv2-load">Nenhuma OS aberta encontrada.</div>' : ''}
                </div>
              </section>
  ...
  async function openHome(root) {
    css();
    root.innerHTML = '<div class="opv2"><section class="opv2-card"><div class="opv2-load">Carregando OS abertas, colaboradores e pontos de embarque...</div></section></div>';

    try {
      await load();
      st.rota = rows().base[0]?.id || '';
      render(root);
      console.info('[opv11] direcionamento carregado', {
      ...
      console.error('[opv11] erro ao carregar direcionamento:', err);
      renderErro(root, err);
    }
  }
