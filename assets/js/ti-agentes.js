...
const AGENTES = [
  ...
  { id: 'sync-mapa-embarque', name: 'Mapa de Embarque', freq: 'fila fixa', table: 'grm_mapa_embarque_importacoes', apiStatus: 'api', apiNote: 'Migrado 05/09: chama a API interna do Graint direto (grmserver-mapa-embarque-api.js, manager/boardPanel/getDayBoardingData). Também corrigido bug pré-existente: a chamada à Edge Function de alertas sempre voltava 401 em produção (supabase.functions.invoke() não mandava o header certo); trocada por fetch cru. Script Puppeteer antigo mantido no disco pra rollback.' },
  ...
  // APROVADO pelo Operador de Logística) — ver grmserver-abrir-os-api.js. Sem
  // card aqui até 10/09 (só existia na Fila V2); validado ao vivo nesta data
  ...
  // por posição na fila com eles). Migrado pra API direta em 12/09
  // (grmserver-abrir-os-api.js, POST serviceOrder/setRecord — devolve o
  // número da O.S. direto na resposta). Script Puppeteer antigo removido em
  // 15/09 (grm-sync-abrir-os.js, já validado e sem uso há 3 dias).
  { id: 'sync-abrir-os', name: 'Abertura de O.S. (GRM)', freq: 'sob demanda (fila saida_abertura_os)', table: 'grm_abertura_os_execucoes', direction: 'saida', apiStatus: 'api', apiNote: 'Migrado 12/09: chama a API interna do Graint direto (grmserver-abrir-os-api.js, serviceOrder/setRecord). Script Puppeteer antigo removido em 15/09.' },
