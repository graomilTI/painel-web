...
async function salvarSolicitacaoEPI(modal,userContext,getColab,colabInput){
  ...
  try{
    ...
    const funcaoSelecionada=modal.querySelector('#solFuncao')?.value?.trim();
    if(!funcaoSelecionada) throw new Error('Selecione a função Operacional I ou Operacional II.');
    ...
    const colab={...(await buscarColaboradorDetalhes(bruto)),funcao:funcaoSelecionada};
