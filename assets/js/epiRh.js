...
async function baixarFichaEpi(s,itens){
  ...
  let y=16;
  const texto=(valor,x,linha,opts={})=>doc.text(String(valor??''),x,linha,opts);
  ...
  doc.rect(margem,y,largura,18); texto(`Razão Social: ${EMPREGADOR_EPI.razao}`,margem+3,y+5); texto(`CNPJ: ${EMPREGADOR_EPI.cnpj}`,125,y+5);
  texto(`Endereço: ${EMPREGADOR_EPI.endereco}`,margem+3,y+12); y+=18;
  secao('FUNCIONÁRIO');
  doc.rect(margem,y,largura,27); texto(`Nome: ${colab.nome||'-'}`,margem+3,y+5); texto(`CPF: ${colab.cpf||'-'}`,margem+3,y+12); texto(`RG: ${colab.rg||'-'}`,85,y+12); texto(`Data Nasc.: ${brDate(colab.data_nascimento)}`,135,y+12);
  texto(`Função: ${colab.funcao||'-'}`,margem+3,y+20); texto(`Cargo: ${colab.cargo||'-'}`,75,y+20); texto(`Setor: ${colab.setor||'-'}`,135,y+20); y+=27;
  ...
  epis.slice(0,12).forEach(item=>{doc.rect(margem,y,largura,8);colunas.slice(1,-1).forEach(x=>doc.line(x,y,x,y+8));texto(item.material||'',colunas[0]+2,y+5);texto(item.ca||'',colunas[1]+2,y+5);texto(item.quantidade||item.unidade||'',colunas[2]+2,y+5);texto('____/____/________',colunas[3]+2,y+5);y+=8;});
  y+=5; secao('RECIBO');
