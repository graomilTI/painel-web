...
function rowDate(row) {
  return isoDate(pick(row, ['Data', 'Data OS', 'Data da OS', 'Data NHE', 'Data de Produção']));
}

function movementUpdateDate(row) {
  return isoDate(pick(row, [
    'Última Atualização',
    'Ultima Atualizacao',
    'Última Atualizacao',
    'Ultima Atualização',
...
async function fetchFirstCompleteBatch(table, startIso, endIso, maxRows, minUniqueOs = 20) {
  let cursor = startIso;
  ...
  for (let attempt = 0; attempt < 16; attempt += 1) {
    ...
    const firstAt = firstRows?.[0]?.created_at;
    if (!firstAt) return { rows: [], batchAt: null };

    const batchEnd = new Date(new Date(firstAt).getTime() + 5 * 60 * 1000).toISOString();
    const rows = await fetchPaged((from, to) => supabase
      .from(table)
      .select('dados_json,created_at')
      .gte('created_at', firstAt)
      .lt('created_at', batchEnd)
      .order('created_at', { ascending: true })
      .range(from, to), maxRows);

    if (latestByOs(rows).size >= minUniqueOs) return { rows, batchAt: firstAt };
    cursor = batchEnd;
  }

  return { rows: [], batchAt: null };
}

// O fechamento usado pela planilha não é o Mapa vivo atual. Ele é a primeira
// fotografia completa capturada no dia seguinte, antes de os embarques do novo
// dia alterarem Tons Hoje e Última Atualização. A referência do FOB vem de
// Última Atualização, e não da coluna Data do mapa.
async function fetchMovementClosingSnapshot() {
  const warnings = [];
  const { startIso, endIso } = todayBounds();

  try {
    const batch = await fetchFirstCompleteBatch(
      'grm_mapa_embarque_importacoes',
      startIso,
      endIso,
      MAX_MAPA,
      20,
    ...
    if (!batch.rows.length) {
      warnings.push(`Mapa de Embarque: nenhuma fotografia de fechamento foi localizada hoje para apurar ${referenceBr()}.`);
      return { rows: [], warnings, batchAt: null, rawCount: 0 };
    }

    const filtered = [];
    latestByOs(batch.rows).forEach((item) => {
      if (movementUpdateDate(item.row) === referenceIso()) filtered.push(item);
    });

    if (!filtered.length) {
      warnings.push(`Mapa de Embarque: a primeira fotografia de hoje não contém O.S. com Última Atualização em ${referenceBr()}.`);
    }

    return {
      rows: filtered.map((item) => ({ dados_json: item.row, created_at: item.createdAt, normalized: true })),
      warnings,
      batchAt: batch.batchAt,
      rawCount: latestByOs(batch.rows).size,
    };
  } catch (error) {
    console.warn('[FOB] fotografia de fechamento', error);
    warnings.push(`Mapa de Embarque: falha ao carregar a fotografia de fechamento (${error.message || error}).`);
    return { rows: [], warnings, batchAt: null, rawCount: 0 };
  }
}
...
async function fetchAgentReferenceDay(table, maxRows, label) {
  ...
  const runWindow = async (fromDate, toDate) => {
...
async function fetchLatestProductionReferenceDay() {
  ...
  try {
    ...
    if (!latestAt) return { rows: [], warnings: ['Produção Diária: nenhum lote sincronizado foi localizado.'] };
    ...
    const rows = batch.filter((record) => {
      const row = normalizedRow(record.dados_json || {});
    ...
    warnings.push(`Produção Diária: a leitura do último lote falhou (${error.message || error}). A comparação seguirá usando NHE.`);
    return { rows: [], warnings };
  }
}
...
async function fetchOperationalOs() {
  try {
    const rows = await fetchPaged((from, to) => supabase
      .from('operacional_os')
      .select('numero_os,cliente,embarque,supervisao,remanescente,lote')
      .range(from, to), 10000);
    return { rows, warning: '' };
  } catch (error) {
...
function buildReport(movement, productionRecords, nheRecords, operationalRows) {
  const mapaByOs = new Map();
  movement.rows.forEach((record) => {
    const row = record.normalized ? record.dados_json : normalizedRow(record.dados_json || {});
    const os = normOs(pick(row, ['O.S.', 'OS', 'O.S', 'O S']));
    if (os) mapaByOs.set(os, { row, createdAt: record.created_at });
  });
  ...
  const prodRows = productionRecords
    .map((record) => normalizedRow(record.dados_json || {}))
    .filter((row) => rowDate(row) === referenceIso() && isFob(row));
  ...
  prodRows.forEach((row) => {
    ...
    if (!os) return;
    prodNheOs.add(os);
    prodNheOsDate.add(`${os}|${referenceIso()}`);
  });
  ...
  mapaByOs.forEach(({ row }) => {
    ...
    const key = `${normalize(cliente)}|${normalize(cidade)}|${normalize(local)}|${referenceIso()}`;
    comboCount.set(key, (comboCount.get(key) || 0) + 1);
  });

  const rows = [];
  mapaByOs.forEach(({ row }, os) => {
    if (movementUpdateDate(row) !== referenceIso()) return;
    ...
    const funcionario = pick(row, ['Atualizado por', 'Atualizado Por', 'Classificador', 'Funcionário', 'Funcionario']);
    const keyOsDate = `${os}|${referenceIso()}`;

    let status = 'PENDENTE';
    if (nheOsDate.has(keyOsDate) || nheOs.has(os) || prodNheOsDate.has(keyOsDate) || prodNheOs.has(os)) {
      status = 'OK';
    } else {
      const combo = `${normalize(cliente)}|${normalize(cidade)}|${normalize(local)}|${referenceIso()}`;
  ...
  return {
    rows,
    stats: {
      mapaBruto: movement.rawCount,
      mapaFechamento: mapaByOs.size,
...
function injectStyles() {
  if (document.getElementById('fob-v6-styles')) return;
  const style = document.createElement('style');
  style.id = 'fob-v6-styles';
  style.textContent = `
  ...
  content.innerHTML = `
    <section class="card mt-16">
      <div class="section-head">
        <div><h3>FOB — Fechamento do dia anterior</h3><p class="muted">Usa a primeira fotografia completa do Mapa capturada hoje e considera somente O.S. cuja Última Atualização ocorreu ontem.</p></div>
      ...
      <div class="fob-note">A coluna <strong>Data</strong> do Mapa representa a fotografia atual. Para o fechamento FOB, a data válida é <strong>Última Atualização</strong>. Entra somente O.S. com Última Atualização em ${referenceBr()} e Tons Hoje = 0.</div>
      <div id="fobFeedback" class="feedback mt-16">Carregando fechamento de ${referenceBr()}...</div>
...
function renderResult() {
  ...
  if (!state.rows.length) {
    host.innerHTML = `<div class="fob-empty">Nenhuma O.S. com Última Atualização em ${referenceBr()} e Tons Hoje = 0 foi localizada na fotografia de fechamento.</div>`;
    return;
  }
  ...
  host.innerHTML = `
    ...
    <div class="fob-diag"><span>Referência: ${referenceBr()}</span><span>Fotografia: ${esc(batchText)}</span><span>Mapa bruto: ${BR_INT.format(stats.mapaBruto || 0)} O.S.</span><span>Atualizadas ontem: ${BR_INT.format(stats.mapaFechamento || 0)} O.S.</span><span>Produção FOB: ${BR_INT.format(stats.producao || 0)}</span><span>NHE: ${BR_INT.format(stats.nhe || 0)}</span></div>
...
async function generateReport() {
  ...
  if (feedback) feedback.textContent = `Localizando a fotografia de fechamento de ${referenceBr()}...`;

  try {
    const [movement, production, nhe, operational] = await Promise.all([
      fetchMovementClosingSnapshot(),
      fetchLatestProductionReferenceDay(),
      fetchAgentReferenceDay('grm_nhe_importacoes', MAX_NHE, 'NHE'),
      fetchOperationalOs(),
    ]);

    state.warnings.push(...movement.warnings, ...production.warnings, ...nhe.warnings, operational.warning);
    const report = buildReport(movement, production.rows, nhe.rows, operational.rows);
    ...
    console.error('[FOB fechamento]', error);
    state.rows = [];
    state.stats = null;
    renderResult();
