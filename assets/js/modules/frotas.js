...
(function () {
  ...
  function markImportedRowsOkByDate(root) {
    ...
    markImportedRowsOk(root, rows, `OK manual aplicado em lote por data (${formatDateBR(targetDate)}).`, currentRenderOpts);
  }
  ...
  async function sincronizarRelatorioBFleet(root, opts = {}) {
    ...
    try {
      ...
      const dataInicial = root.querySelector('[data-bfleet-sync-start]')?.value || '';
      const dataFinal = root.querySelector('[data-bfleet-sync-end]')?.value || '';
      const body = { mode: 'sync', forceRefreshToken: true, preferWebReport: true };
      if (dataInicial || dataFinal) {
        body.dataInicial = dataInicial || dataFinal;
        body.dataFinal = dataFinal || dataInicial;
        body.startDate = body.dataInicial;
        body.endDate = body.dataFinal;
        body.rangeTimeVal = '';
      } else {
        body.rangeTimeVal = 'yesterday';
      }
      const periodoMsg = body.dataInicial || body.dataFinal
        ? `${formatDateBR(body.dataInicial)} a ${formatDateBR(body.dataFinal)}`
        : 'yesterday';
      toast(`Sincronizando relatório de excesso de velocidade da BFleet (${periodoMsg})...`);
              ...
              <div class="speed-panel">
                ...
                <div class="speed-import-card">
                  ...
                  <div class="speed-sync-range">
                    <div class="speed-field"><label>Data inicial para sincronizar</label><input class="speed-input" type="date" data-bfleet-sync-start></div>
                    <div class="speed-field"><label>Data final para sincronizar</label><input class="speed-input" type="date" data-bfleet-sync-end></div>
                  </div>
                  <p class="speed-hint">Se deixar as datas vazias, a sincronização usa o marcador padrão <strong>yesterday</strong>. Para buscar um período específico, preencha Data inicial e Data final antes de clicar em Sincronizar.</p>
