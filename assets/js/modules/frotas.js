...
(function () {
  ...
  function makePastedFile(item, index) {
    const blob = item instanceof File ? item : null;
    if (!blob) return null;
    const ext = (blob.type || 'image/png').includes('jpeg') ? 'jpg'
      : (blob.type || 'image/png').includes('webp') ? 'webp'
      : 'png';
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const name = blob.name && blob.name !== 'image.png' ? blob.name : `print-colado-${stamp}-${index + 1}.${ext}`;
    try {
  ...
  function addPrintFiles(root, files, sourceLabel = 'selecionado') {
    const incoming = Array.from(files || [])
      .filter((file) => String(file?.type || '').startsWith('image/'))
      .map((file, index) => makePastedFile(file, index))
      .filter(Boolean);

    if (!incoming.length) return false;

    state.uploadedFiles = [...state.uploadedFiles, ...incoming];
    renderUploadLists(root);
    toast(`${incoming.length} print(s) ${sourceLabel}(s) adicionados.`);
    return true;
  }

  function handlePastePrints(root, ev) {
    const items = Array.from(ev.clipboardData?.items || []);
    const files = items
      .filter((item) => item.kind === 'file' && String(item.type || '').startsWith('image/'))
      .map((item, index) => makePastedFile(item.getAsFile(), index))
      .filter(Boolean);

    if (!files.length) return;
    ev.preventDefault();
    addPrintFiles(root, files, 'colado');

    const pasteBox = root.querySelector('[data-paste-prints]');
    if (pasteBox) {
      pasteBox.classList.add('active');
      window.setTimeout(() => pasteBox.classList.remove('active'), 800);
    }
  }
    ...
    try {
      ...
      const resp = await fetch(gasUrl, {
        ...
        body: JSON.stringify({
          ...
          fileNameTemplate: '{{N}}º NOTIFICAÇÃO DE VELOCIDADE {{YYYY}} {{CONDUTOR}}',
          expectedFileNameExample: `1º NOTIFICAÇÃO DE VELOCIDADE ${anoNotificacao} CONDUTOR`,
          sequentialScope: 'driver_folder',
          disableLegacyFileName: true,
          removeDatePrefixFromFileName: true,
          removePlateFromFileName: true,
          files
        })
      });
  ...
  function renderExcessoVelocidade(container, opts = {}) {
          ...
          <div class="frotas-body">
            <div class="speed-grid">
              <div class="speed-panel">
                ...
                <div class="speed-field"><label>Data da notificação</label><input class="speed-input" type="text" value="${escapeHtml(todayBRShort())}" data-notification-date><p class="speed-hint">Usada para definir o ano da notificação. No Drive será salvo como: <code>(N)º NOTIFICAÇÃO DE VELOCIDADE ANO CONDUTOR</code></p></div>
                ...
                <div class="upload-box">
                  ...
                  <div class="paste-capture" tabindex="0" data-paste-prints><strong>Colar print direto aqui</strong><span>Clique nesta área e pressione <b>Ctrl + V</b> logo após tirar o print. Também funciona colando em qualquer ponto desta tela.</span></div>
                  ...
                  <p class="speed-hint">Os prints não ficam associados ao colaborador selecionado acima. O Web App recebe os arquivos em lote e deve organizar cada print pelo OCR, usando o padrão: <code>(N)º NOTIFICAÇÃO DE VELOCIDADE ${extractYearFromDate(todayBRShort())} CONDUTOR</code>.</p>
    ...
    container.querySelector('[data-print-files]')?.addEventListener('change', (ev) => { addPrintFiles(container, Array.from(ev.target.files || []), 'selecionado'); });
    container.addEventListener('paste', (ev) => handlePastePrints(container, ev));
    container.querySelector('[data-paste-prints]')?.addEventListener('click', (ev) => ev.currentTarget.focus());
