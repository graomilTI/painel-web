(function(){
  ...
  const state={multas:[], motoristas:[], motoristasLoaded:false, busca:'', filtro:'abertas', filtro2:'todas', arquivo:'ativas', sortKey:'vencimento', sortDir:'desc'};
  ...
  async function loadMotoristas(opts){
    ...
    for(const sel of selects){
      try{
        const {data,error}=await opts.supabase.from('colaborador_snapshot').select(sel).order('nome',{ascending:true}).limit(3000);
        if(error) throw error;
        state.motoristas=uniqueMotoristas((data||[]).filter(isColaboradorAtivo));
  ...
  function filtered(){
    ...
    return state.multas.filter(m=>{
      ...
      if(state.filtro==='a_pagar'&&!isAPagar(m))return false;
  ...
  function openHome(container,opts={}){
    container.innerHTML=`${styles}<section class="fm-shell"><div class="fm-head"><div class="fm-kicker">Frotas · Notificações</div><h1 class="fm-title">Multas</h1><p class="fm-sub">Consulta, conferência, definição de motorista, identificação/dobra e arquivamento das multas recentes.</p></div><div class="fm-card"><div class="fm-tabs"><button class="fm-tab" data-open-dashboard>Dashboard</button><button class="fm-tab" data-open-excesso>Excesso de Velocidade</button><button class="fm-tab" data-open-veiculos>Veículos</button><button class="fm-tab active">Multas</button><button class="fm-tab" data-open-historico>Histórico</button></div><div class="fm-body"><div class="fm-toolbar"><select class="fm-select" data-filter><option value="abertas">Abertas</option><option value="vencidas">Vencidas</option><option value="a_pagar">A pagar</option><option value="todas">Todas</option></select><select class="fm-select" data-filter2><option value="todas">Todas situações</option><option value="pendentes">Pendentes (sem motorista)</option><option value="identificadas">Identificadas</option><option value="dobradas">Dobradas</option></select><select class="fm-select" data-archive-filter><option value="ativas">Multas ativas</option><option value="arquivadas">Consultar arquivadas</option><option value="todas">Ativas + arquivadas</option></select><input class="fm-input" data-search placeholder="Buscar por placa, motorista, auto, renavam, empresa..."><button class="fm-btn primary" data-refresh>Atualizar</button><button class="fm-btn primary" data-sync-multas>Sincronizar DETRAN</button></div><div class="fm-grid"><div class="fm-kpi"><span>Abertas</span><strong data-kpi-abertas>0</strong></div><div class="fm-kpi"><span>Vencidas</span><strong data-kpi-vencidas>0</strong></div><div class="fm-kpi"><span>Valor filtrado</span><strong data-kpi-valor>R$ 0,00</strong></div><div class="fm-kpi"><span>Guias/PDFs</span><strong data-kpi-guias>0</strong></div></div><p class="fm-sub" data-count>0 multa(s) encontrada(s)</p><div class="fm-table-wrap"><table class="fm-table"><thead><tr><th><button class="fm-sort" data-sort="placa" data-label="Placa / Empresa">Placa / Empresa</button></th><th><button class="fm-sort" data-sort="infracao" data-label="Infração">Infração</button></th><th><button class="fm-sort" data-sort="motorista" data-label="Motorista">Motorista</button></th><th><button class="fm-sort" data-sort="descricao" data-label="Descrição / Local">Descrição / Local</button></th><th><button class="fm-sort" data-sort="numero_auto_infracao" data-label="Auto">Auto</button></th><th><button class="fm-sort" data-sort="valor" data-label="Valor">Valor</button></th><th><button class="fm-sort" data-sort="vencimento" data-label="Vencimento">Vencimento</button></th><th><button class="fm-sort" data-sort="status" data-label="Status">Status</button></th><th>Ações</th></tr></thead><tbody data-multas-table></tbody></table></div><div class="fm-note">A lista exibe somente multas dos últimos <strong>180 dias</strong>. Multas com <strong>vencimento anterior a hoje</strong> são arquivadas automaticamente para manter a tela leve para novas multas.</div></div></div></section>`;
