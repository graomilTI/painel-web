(function () {
  ...
  const styles = `
    <style>
      ...
      .dre-controls{display:flex;gap:10px;flex-wrap:wrap;align-items:center;justify-content:flex-end}.dre-controls select,.dre-controls button{height:42px;border-radius:14px;border:1px solid rgba(255,255,255,0.08);background:#0d0d18;color:#e2e2f0;padding:0 12px;font-weight:850;color-scheme:dark}.dre-controls button{cursor:pointer}.dre-controls button.primary{background:linear-gradient(135deg,#166534,#22c55e);color:#052e16;border:0}.dre-controls button:disabled{opacity:.5;cursor:not-allowed}.dre-toolbar{display:flex;gap:12px 16px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-bottom:14px}.dre-tabs{display:flex;gap:8px;flex-wrap:wrap}.dre-tab{height:42px;border:1px solid var(--line);background:#0d0d18;color:#cbd5e1;border-radius:999px;padding:0 15px;cursor:pointer;font-weight:900}.dre-tab.active{background:#166534;color:#fff;border-color:#22c55e}
      ...
      @media(max-width:1180px){.dre-cards{grid-template-columns:repeat(3,minmax(0,1fr))}.dre-grid{grid-template-columns:1fr}.dre-controls{justify-content:flex-start}}@media(max-width:720px){.dre-cards{grid-template-columns:1fr}.dre-controls select,.dre-controls button{width:100%}}
    </style>`;
  ...
  function openHome(container, opts={}){
    ...
    container.innerHTML=`${styles}<section class="dre-wrap"><div class="dre-toolbar"><div class="dre-tabs"><button class="dre-tab active" data-tab="geral">DRE Geral</button><button class="dre-tab" data-tab="regional">DRE Regional</button></div><div class="dre-controls"><select id="yearSelect">${[state.year-1,state.year,state.year+1].map(y=>`<option value="${y}" ${y===state.year?'selected':''}>${y}</option>`).join('')}</select><select id="regionalSelect" disabled></select><button id="refreshDre" class="primary">Atualizar agora</button><button id="exportPdf" disabled>PDF</button><button id="exportAllPdfs" disabled>PDFs Regionais</button><button id="exportImg" disabled>Imagem</button></div></div><div class="dre-status show" id="dreStatus"><strong>Carregando...</strong></div><div id="dreCards" class="dre-cards"></div><div id="dreCharts" class="dre-grid"></div><article class="dre-report" id="dreReport"><div class="dre-report-head"><div><h3>DRE</h3><p>Sem dados processados.</p></div></div></article></section>`;
