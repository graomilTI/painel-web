(function () {
  ...
  const styles = `
    <style>
      ...
      .dre-report{border:1px solid rgba(134,239,172,.24);border-radius:28px;overflow:hidden;background:linear-gradient(180deg,#f8fffb,#eefbf3);color:#0f172a;box-shadow:0 28px 70px rgba(0,0,0,.22)}.dre-report-head{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:20px 22px;background:radial-gradient(circle at 92% 0%,rgba(86,201,122,.26),transparent 34%),linear-gradient(135deg,#052e16,#0d5a2a 58%,#166534);color:#fff}.dre-report-head h3{margin:0;font-size:22px;letter-spacing:-.035em}.dre-report-head p{margin:5px 0 0;color:#dfffe9;font-size:12px;font-weight:750}.dre-table-wrap{overflow:auto;background:#f8fffb;max-height:72vh}.dre-table{width:100%;border-collapse:separate;border-spacing:0;font-size:12px;min-width:1180px}.dre-table th{position:sticky;top:0;z-index:3;background:linear-gradient(180deg,#dff7e6,#c4edd2);color:#052e16;text-align:center;font-weight:950;padding:12px 10px;border-right:1px solid #b7d9c3;border-bottom:1px solid #a9cfb8;text-transform:uppercase;font-size:11px}.dre-table th:first-child,.dre-table td:first-child{position:sticky;left:0;z-index:4;text-align:left}.dre-table th:first-child{background:linear-gradient(180deg,#eefbf3,#d8f4e2)}.dre-table td{padding:10px 12px;border-right:1px solid #d6e3dc;border-bottom:1px solid #d6e3dc;text-align:right;white-space:nowrap;background:#ffffff}.dre-table td:first-child{font-weight:950;color:#0f172a;background:#ffffff;box-shadow:8px 0 16px rgba(15,23,42,.05)}.dre-table tr:nth-child(even) td{background:#f4fbf7}.dre-table tr:nth-child(even) td:first-child{background:#f4fbf7}.dre-table tbody tr:hover td{background:#ecfdf3!important}.dre-table tr.highlight td{background:#daf7e4!important;color:#064e3b;font-weight:950}.dre-table tr.result td{background:linear-gradient(180deg,#bbf7d0,#a7f3c2)!important;color:#052e16;font-weight:950}.dre-table .neg{color:#ef1f1f;font-weight:950}.dre-extra{display:grid;gap:12px;padding:14px;background:#f8fffb}.dre-extra-box{border:1px solid #c9ded2;border-radius:18px;overflow:hidden;background:#fff}.dre-extra-box h4{margin:0;padding:11px 13px;background:linear-gradient(180deg,#dff7e6,#c4edd2);color:#052e16}.dre-extra-box table{width:100%;border-collapse:collapse;font-size:12px}.dre-extra-box td,.dre-extra-box th{border:1px solid #d6e3dc;padding:9px;text-align:right}.dre-extra-box td:first-child,.dre-extra-box th:first-child{text-align:left;font-weight:850}
  ...
  function openHome(container, opts={}){
    ...
    container.querySelector('#yearSelect').addEventListener('change',e=>{state.year=Number(e.target.value)||state.year; if(typeof loadDre==='function') loadDre();});
    ...
    const loadDre=async()=>{
      ...
      try{
        ...
        console.error(err);
        setStatus(`<strong>Erro:</strong> ${safe(err?.message||'Falha ao carregar DRE automaticamente.')}`);
      }finally{
        btn.disabled=false;
        btn.textContent='Atualizar DRE';
      }
    };
    container.querySelector('#refreshDre').addEventListener('click',loadDre);
    loadDre();
