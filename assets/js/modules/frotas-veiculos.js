(function () {
  ...
  async function sincronizarFrota(root, opts){
    try{
      ...
      toast(`Veículos atualizados: ${totalVeiculos}. Sincronizando multas...`);
      const multas = await callFunction(opts, 'sync-multas-detran', { mode:'all' });
      const totalMultas = Number(multas?.total_multas || multas?.total || 0);
      toast(`Sincronização concluída: ${totalVeiculos} veículo(s) e ${totalMultas} multa(s).`);
      await loadVeiculos(root, opts);
