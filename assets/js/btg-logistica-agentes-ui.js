...
let rows=[], pontos=[], all={btg:0,lista:0,dist:0,pontos:0};
...
async function load(){
  ...
  pontos=agruparPontos(ops.error?[]:(ops.data||[]));
  all={btg:rows.length,lista,dist,pontos:pontos.length};
}
function agruparPontos(data){
  const m=new Map();
  for(const r of data){
    const ponto=(r.embarque||r.destino||'SEM PONTO').trim();
    const key=norm(ponto)||'SEM PONTO';
    if(!m.has(key))m.set(key,{ponto,os:0,volume:0,clientes:new Set(),produtos:new Set(),supervisoes:new Set()});
    const g=m.get(key); g.os++; g.volume+=Number(r.remanescente||r.lote||0)||0; if(r.cliente)g.clientes.add(r.cliente); if(r.produto)g.produtos.add(r.produto); if(r.supervisao)g.supervisoes.add(r.supervisao);
  }
...
function render(el){
  const q=norm(el.busca.value), list=rows.filter(r=>!q||norm(`${r.contrato} ${r.portal} ${r.cliente} ${r.produto} ${status(r)}`).includes(q)), pts=pontos.filter(p=>!q||norm(`${p.ponto} ${p.clientes.join(' ')} ${p.produtos.join(' ')} ${p.supervisoes.join(' ')}`).includes(q));
  const pend=list.filter(r=>status(r)!=='OK').length;
  el.kpis.innerHTML=`<div class=k><span>Relatório BTG</span><b>${all.btg}</b></div><div class=k><span>Lista OS</span><b>${all.lista}</b></div><div class=k><span>Distribuição</span><b>${all.dist}</b></div><div class=k><span>Pontos BTG</span><b>${all.pontos}</b></div><div class="k ${pend?'warn':'ok'}"><span>Pendências</span><b>${pend}</b></div>`;
  el.feedback.textContent=`${list.length} relatório(s) e ${pts.length} ponto(s) exibidos.`;
