...
(function () {
  ...
  const STYLE_ID = 'opv4-direcionamento-style';
  const LEAFLET_CSS = 'leaflet-css-opv4';
  ...
  async function sel(table, columns = '*', fn = null, limit = 5000) {
    try {
      ...
      console.warn(`[opv4] ${table}:`, e?.message || e);
      return [];
    }
  }
  ...
  function css() {
    ...
    s.textContent = `
      ...
      .opv2-row.active,.opv2-row:hover{border-color:#22c55e;background:rgba(22,101,52,.13)}.opv2-row.distante{border-color:rgba(251,191,36,.32)}
  ...
  function draw() {
    ...
    if (r && geo(r.colab) && geo(r.ponto)) {
      ...
      L.polyline(pts, { color: r.distante ? '#f59e0b' : (r.carona ? '#f59e0b' : '#22c55e'), weight: 4, opacity: .9, dashArray: r.distante || !r.colab.temFrota ? '7 7' : null }).addTo(st.layer);
