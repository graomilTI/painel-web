...
function ensureGacDropdown() {
  ...
  document.addEventListener('mousedown', (event) => {
    ...
    if (!gacDropdownEl.contains(event.target) && event.target !== gacState.input) hideGacDropdown();
  });

  const reposition = () => {
    if (gacDropdownEl && !gacDropdownEl.hidden && gacState.input) positionGacDropdown(gacDropdownEl, gacState.input);
  };
...
function fmtTon(value) { return BR.format(num(value)); }
...
function onlyActiveColab(c) { return !!c && c.ativo !== false; }
...
function statusLabel(row) { return normalize(row?.status_gestor) || 'PENDENTE'; }
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    @media (min-width:901px){.os-lite-list{display:flex;flex-direction:column;gap:8px}.os-lite-card{display:grid;grid-template-columns:minmax(170px,1.05fr) minmax(280px,1.7fr) minmax(220px,1.15fr) minmax(190px,.95fr);align-items:stretch;border-radius:12px;overflow:visible}.os-lite-row{display:flex;min-width:0;padding:9px 11px;border-bottom:0;border-right:1px solid rgba(148,163,184,.1)}.os-lite-row:last-child{border-right:0}.os-lite-head{align-items:center}.os-lite-client,.os-lite-indic{flex-direction:column;justify-content:center}.os-lite-footer{align-items:center;justify-content:center;flex-direction:column;gap:7px}.os-lite-rembox{min-width:96px}.os-lite-buttons{justify-content:center;flex-wrap:nowrap}.os-lite-gac-input{min-height:34px}}
  `;
...
function buildQuery(supervisao, status, from, to) {
  ...
  if (statusNorm === 'PENDENTE') query = query.or('status_gestor.is.null,status_gestor.eq.PENDENTE');
  else if (statusNorm) query = query.eq('status_gestor', statusNorm);
  return query;
}
...
async function loadAtribuicoes(osIds) {
  ...
  const results = await Promise.all(chunks.map((chunk) => (
    supabase.from('operacional_os_colaboradores').select('id,os_id,colaborador_key,colaborador_nome,distancia_km,origem_sugestao').in('os_id', chunk)
  )));
...
async function loadPontos() {
  ...
  const { data, error } = await supabase.from('operacional_pontos_embarque').select(PONTO_SELECT).eq('ativo', true).limit(8000);
  if (error) {
...
async function loadColabs(supervisao) {
  ...
  const results = supervisao
    ? await Promise.all([base().eq('supervisao', supervisao), base().eq('coordenacao', supervisao), base()])
    : await Promise.all([base()]);
  const erro = results.find((r) => r.error);
  if (erro) {
    console.warn('Falha ao carregar colaboradores.', erro.error);
    colabsCache.set(key, []);
    return [];
  }
  const map = new Map();
  results.forEach((r) => (r.data || []).forEach((c) => map.set(colabKey(c), c)));
  const rows = [...map.values()].filter(onlyActiveColab);
  colabsCache.set(key, rows);
  return rows;
}

function pontoDiretoDaOs(row) {
  if (hasGeo(row?.ponto1_latitude, row?.ponto1_longitude)) {
    return {
      id: `os-${row.id}`,
      nome_local: row.ponto1_nome || row.embarque || 'Ponto da OS',
      cidade: splitUfCidadeLocal(row.embarque).cidade,
      uf: splitUfCidadeLocal(row.embarque).uf,
...
function bestPontoForOs(row, pontos) {
  const direto = pontoDiretoDaOs(row);
  ...
  }).filter((x) => x.score >= 120 && hasGeo(x.ponto.latitude, x.ponto.longitude)).sort((a, b) => b.score - a.score);
...
async function sugerirColaborador(row, atribuicoes) {
  ...
  const [pontos, colabs] = await Promise.all([loadPontos(), loadColabs(row.supervisao)]);
  const ponto = bestPontoForOs(row, pontos);
  if (!ponto || !hasGeo(ponto.latitude, ponto.longitude)) return null;
...
function rowHtml(row, atribuicoes) {
  ...
  return `<article data-os-id="${escapeHtml(row.id)}" class="os-lite-card ${zero ? 'os-lite-zero' : ''} ${rowColorClass}">
    <div class="os-lite-row os-lite-head"><div><div class="os-lite-title">${escapeHtml(row.numero_os)}</div><div class="os-lite-meta">${brDate(row.data_os)} · ${escapeHtml(row.servico || '-')} · ${escapeHtml(row.supervisao || '-')}</div></div><div class="os-lite-rembox"><span class="os-lite-chip ${zero ? 'warn' : rem <= 555000 ? 'info' : 'ok'}">${fmtTon(rem)}</span><div class="os-lite-meta">Lote ${fmtTon(row.lote)} · Emb. ${fmtTon(row.embarcado)}</div></div></div>
    <div class="os-lite-row os-lite-client"><div class="os-lite-title">${escapeHtml(row.cliente || '-')}</div><div class="os-lite-meta os-lite-route">Emb.: ${escapeHtml(row.embarque || '-')} → Dest.: ${escapeHtml(row.destino || '-')}</div><div class="os-lite-meta os-lite-route">Contrato ${escapeHtml(row.contrato || '-')} • ${escapeHtml(row.produto || '-')}</div></div>
    <div class="os-lite-row os-lite-indic"><input type="text" class="os-lite-gac-input" value="${escapeHtml(main?.colaborador_nome || '')}" placeholder="Selecionar colaborador..." autocomplete="off" spellcheck="false" /><div class="os-lite-meta">${main?.distancia_km != null ? `${KM.format(main.distancia_km)} km` : main ? 'Indicado' : 'Sem indicação. Ao marcar Atender, o painel tenta sugerir pelo mapa operacional.'}</div></div>
    <div class="os-lite-row os-lite-footer"><div class="os-lite-buttons">${statusButtons}${kgButton}${conferirButton}</div><span class="os-lite-chip ${statusClass(row)}">${escapeHtml(status)}</span></div>
  </article>`;
}
...
export async function renderOsProgramacaoLite(content, options = {}) {
  ...
  const state = { supervisao: String(options.supervisao || '').trim(), status: String(options.status || '').trim(), rows: [], atribuicoes: [], total: 0, loadingMore: false };
  if (!currentUser) currentUser = await getCurrentUser().catch(() => null);

  content.innerHTML = `<div id="osLiteRoot"><div class="os-lite-actions"><span class="muted" id="osLiteFeedback">Preparando consulta...</span><button type="button" class="btn btn-secondary" id="osLiteReload">↻ Atualizar</button></div><div id="osLiteStats"></div><div id="osLiteList"><div class="os-lite-empty">Carregando...</div></div></div>`;
  const el = { feedback: content.querySelector('#osLiteFeedback'), reload: content.querySelector('#osLiteReload'), stats: content.querySelector('#osLiteStats'), list: content.querySelector('#osLiteList') };
  if (!state.supervisao) {
  ...
  async function loadPage({ append = false } = {}) {
    ...
    try {
      ...
      state.rows = append ? [...state.rows, ...chunk] : chunk;
      state.atribuicoes = append ? [...state.atribuicoes, ...atribuicoes] : atribuicoes;
  ...
  function render() {
    ...
    const more = state.rows.length < state.total ? `<div class="os-lite-load-more"><button type="button" class="btn btn-secondary" id="osLiteMore">Carregar mais ${Math.min(PAGE_SIZE, state.total - state.rows.length)} O.S.</button></div>` : '';
    ...
    try {
      if (nextStatus === 'ATENDER') {
        ...
        if (!atual.length) {
          ...
          if (novo) state.atribuicoes = [...state.atribuicoes.filter((a) => !(String(a.os_id) === String(novo.os_id) && String(a.colaborador_key) === String(novo.colaborador_key))), novo];
          else el.feedback.textContent = `OS ${row.numero_os}: sem sugestão automática, liberando para Conferência sem travar.`;
        }
      }
      const agoraIso = new Date().toISOString();
      const patch = { status_gestor: nextStatus, configurada_em: agoraIso, observacao_logistica: null };
  ...
  async function buscarColabsParaOs(row, query) {
    const [colabs, pontos] = await Promise.all([loadColabs(row.supervisao), loadPontos()]);
    const ponto = bestPontoForOs(row, pontos);
  ...
  async function abrirDropdownColaborador(input, row, query) {
    ...
    gacState = { input, row, onSelect: (r, key) => {
    ...
    let results;
    try { results = await buscarColabsParaOs(row, query); }
    catch (error) {
  ...
  async function selecionarColaborador(row, colab) {
    ...
    const payload = { os_id: row.id, colaborador_key: colabKey(colab), colaborador_nome: colab.nome || 'Colaborador', distancia_km: Number.isFinite(distancia) ? distancia : null, origem_sugestao: 'APP_GESTOR_MANUAL_LITE', indicado_por: currentUser?.id || null };
    el.feedback.textContent = `Atualizando colaborador da OS ${row.numero_os}...`;
    try {
      const { data, error } = await supabase.from('operacional_os_colaboradores').upsert(payload, { onConflict: 'os_id,colaborador_key' }).select('id,os_id,colaborador_key,colaborador_nome,distancia_km,origem_sugestao').maybeSingle();
      if (error) throw error;
  ...
  function openKgModalLite(row) {
    ...
    overlay.innerHTML = `<div class="os-lite-kg-modal"><h3>Qual o valor precisa somar na O.S.?</h3><p>O.S. <strong style="color:#bbf7d0">${escapeHtml(row.numero_os)}</strong> — valor será enviado para a Logística.</p><input id="osLiteKgInput" type="number" min="1" placeholder="Inserir KG" inputmode="numeric" /><div class="os-lite-kg-actions"><button type="button" class="btn btn-secondary" id="osLiteKgCancelar">Cancelar</button><button type="button" class="btn btn-primary" id="osLiteKgConfirmar">Confirmar</button></div></div>`;
    ...
    overlay.querySelector('#osLiteKgConfirmar').addEventListener('click', async () => {
      ...
      const kgText = `KG solicitado pelo gestor: ${BR.format(kg)} kg`;
      row.observacao_logistica = kgText;
  ...
  el.reload.addEventListener('click', () => loadPage());
  el.list.addEventListener('click', (event) => {
    const card = event.target.closest('[data-os-id]');
    const row = card ? state.rows.find((item) => String(item.id) === String(card.dataset.osId)) : null;
    const statusBtn = event.target.closest('[data-status]');
    if (statusBtn && row) return atualizarStatus(row.id, statusBtn.dataset.status, statusBtn);
    const kgBtn = event.target.closest('[data-action-kg]');
    if (kgBtn && row) return openKgModalLite(row);
  });
  el.list.addEventListener('input', debounce((event) => {
    if (!event.target.matches('.os-lite-gac-input')) return;
    const card = event.target.closest('[data-os-id]');
    const row = card ? state.rows.find((item) => String(item.id) === String(card.dataset.osId)) : null;
    if (!row) return;
    abrirDropdownColaborador(event.target, row, event.target.value || '');
  }, 180));
