...
const mailboxType = (path) => { const p=norm(path); if(/sent|enviad/.test(p))return'enviados'; if(/draft|rascun/.test(p))return'rascunhos'; if(/trash|lixeira|deleted/.test(p))return'lixeira'; if(/junk|spam/.test(p))return'spam'; return'entrada'; };
...
function renderReader() {
  ...
  return `<div class="gm-reader-tools"><button data-action="read" type="button" title="${m.lido?'Marcar não lido':'Marcar lido'}">${m.lido?'○':'●'}</button><button data-action="star" type="button" title="Favorito">${m.favorito?'★':'☆'}</button><button data-action="archive" type="button" title="Arquivar"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v10"/><path d="m8 9 4 4 4-4"/><path d="M4 15v4a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-4"/></svg></button><button data-action="trash" type="button" title="Excluir">✕</button></div><h3>${esc(m.assunto||'(sem assunto)')}</h3><div class="gm-sender"><span class="gm-avatar">${esc(initials(m.remetente_nome,m.remetente_email))}</span><div><b>${esc(m.remetente_nome||m.remetente_email||'-')}</b><span>${esc(m.remetente_email||'')} · ${when(m.data_recebimento)}</span></div></div><div class="gm-attachments">${state.attachments.map(a=>`<button class="gm-attachment" data-attachment="${esc(a.storage_path||'')}" type="button" ${a.storage_path?'':'disabled'}>📎 ${esc(a.nome_arquivo)}</button>`).join('')}</div><div class="gm-body">${esc(m.corpo_texto||String(m.corpo_html||'').replace(/<[^>]+>/g,' ')||'(sem conteúdo)')}</div><div class="gm-reader-actions"><button class="btn btn-primary" data-compose-mode="reply" type="button">Responder</button><button class="btn btn-secondary" data-compose-mode="forward" type="button">Encaminhar</button><button class="btn btn-secondary" data-abrir-os type="button" title="Lê este e-mail e leva pra Abrir O.S. com os campos já preenchidos">🚚 Abrir O.S.</button></div>`;
}
...
function accountModal(){const s=accountStatus();return`<div class="gm-modal" id="gmAccountModal" hidden><div class="gm-modal-card"><div class="gm-eyebrow">Conta vinculada</div><h3>${esc(state.account.email)}</h3><p class="muted">${s.label}<br>Última sincronização: ${when(state.account.ultima_sync_em)}${s.error?`<br><span class="text-danger">${esc(s.error)}</span>`:''}</p><div class="gm-compose-actions"><button class="btn btn-danger" id="gmDisconnect" type="button">Desconectar e apagar dados</button><button class="btn btn-secondary" data-close-modal type="button">Fechar</button></div></div></div>`}

async function connectAccount(event){event.preventDefault();const feedback=document.getElementById('gmConnectFeedback'),button=event.submitter;button.disabled=true;feedback.textContent='Salvando e preparando a sincronização...';const email=document.getElementById('gmEmail').value.trim();const domain=email.split('@')[1]||'grao1000.com.br';const payload={action:'connect',email,password:document.getElementById('gmPassword').value,provider:document.getElementById('gmProvider').value,username:document.getElementById('gmUsername').value.trim()||email,imap_host:document.getElementById('gmImapHost').value.trim()||`mail.${domain}`,imap_port:Number(document.getElementById('gmImapPort').value||993),smtp_host:document.getElementById('gmSmtpHost').value.trim()||`mail.${domain}`,smtp_port:Number(document.getElementById('gmSmtpPort').value||465)};const{data,error}=await supabase.functions.invoke('gestor-email-account',{body:payload});if(error||!data?.ok){feedback.textContent=data?.error||error?.message||'Falha ao conectar.';button.disabled=false;return}await loadAll(document.getElementById('pageContent'));}

async function loadAccount(){const{data,error}=await supabase.from('email_accounts_public').select('*').eq('escopo','GESTOR').maybeSingle();if(error)throw error;state.account=data||null;}
async function loadMessages(){if(!state.account){state.messages=[];return}const{data,error}=await supabase.from('email_messages').select('id,account_id,mailbox_path,remetente_nome,remetente_email,destinatario,cc,assunto,corpo_texto,corpo_html,data_recebimento,lido,favorito,arquivado_em,excluido_em,status').eq('account_id',state.account.id).order('data_recebimento',{ascending:false}).limit(250);if(error)throw error;state.messages=data||[];if(state.selected)state.selected=state.messages.find(m=>m.id===state.selected.id)||null;}
async function selectMessage(id,content){state.selected=state.messages.find(m=>m.id===id)||null;if(state.selected&&!state.selected.lido){await supabase.from('email_messages').update({lido:true}).eq('id',id);state.selected.lido=true}const{data}=await supabase.from('email_attachments').select('id,nome_arquivo,mime_type,tamanho_bytes,storage_path').eq('email_id',id).order('created_at');state.attachments=data||[];renderWorkspace(content);}

function openCompose(mode='new'){const modal=document.getElementById('gmComposeModal'),m=state.selected;document.getElementById('gmComposeTitle').textContent=mode==='reply'?'Responder':mode==='forward'?'Encaminhar':'Novo e-mail';document.getElementById('gmTo').value=mode==='reply'?(m?.remetente_email||''):'';document.getElementById('gmSubject').value=mode==='reply'?(/^re:/i.test(m?.assunto||'')?m.assunto:`Re: ${m?.assunto||''}`):mode==='forward'?`Enc: ${m?.assunto||''}`:'';document.getElementById('gmBody').value=mode==='forward'?`\n\n---------- Mensagem encaminhada ----------\nDe: ${m?.remetente_email||''}\nAssunto: ${m?.assunto||''}\n\n${m?.corpo_texto||''}`:'';modal.dataset.mode=mode;modal.hidden=false;}

async function sendMessage(event){event.preventDefault();const feedback=document.getElementById('gmComposeFeedback'),mode=document.getElementById('gmComposeModal').dataset.mode||'new';const payload={email_id:mode==='new'?null:state.selected?.id||null,account_id:state.account.id,para:document.getElementById('gmTo').value.trim(),cc:document.getElementById('gmCc').value.trim()||null,assunto:document.getElementById('gmSubject').value.trim(),corpo:document.getElementById('gmBody').value.trim(),status:'PENDENTE',tipo:mode==='reply'?'RESPOSTA':mode==='forward'?'ENCAMINHAMENTO':'NOVO',aprovado_por:state.user.id,aprovado_por_nome:state.user.email,aprovado_em:new Date().toISOString()};const{error}=await supabase.from('email_outbox').insert(payload);if(error){feedback.textContent=error.message;return}document.getElementById('gmComposeModal').hidden=true;alert('Mensagem adicionada à fila de envio.');}

async function messageAction(action,content){const m=state.selected;if(!m)return;let update={};if(action==='read')update={lido:!m.lido};if(action==='star')update={favorito:!m.favorito};if(action==='archive')update={arquivado_em:new Date().toISOString(),mailbox_path:'Archive'};if(action==='trash')update={excluido_em:new Date().toISOString(),mailbox_path:'Trash'};const{error}=await supabase.from('email_messages').update(update).eq('id',m.id);if(error)return alert(error.message);Object.assign(m,update);await loadMessages();state.selected=null;state.attachments=[];renderWorkspace(content);}

function bindWorkspace(content){content.querySelector('#gmSearch').addEventListener('input',e=>{state.search=e.target.value;renderWorkspace(content);content.querySelector('#gmSearch')?.focus()});content.querySelectorAll('[data-folder]').forEach(b=>b.addEventListener('click',()=>{state.folder=b.dataset.folder;state.selected=null;state.attachments=[];renderWorkspace(content)}));content.querySelectorAll('[data-message]').forEach(b=>b.addEventListener('click',()=>selectMessage(b.dataset.message,content)));content.querySelector('#gmCompose').addEventListener('click',()=>openCompose());content.querySelector('#gmComposeForm').addEventListener('submit',sendMessage);content.querySelector('#gmSync').addEventListener('click',async()=>{await supabase.functions.invoke('gestor-email-account',{body:{action:'sync'}});alert('Sincronização solicitada.');});content.querySelector('#gmAccount').addEventListener('click',()=>document.getElementById('gmAccountModal').hidden=false);content.querySelector('#gmFixAccount')?.addEventListener('click',()=>document.getElementById('gmAccountModal').hidden=false);content.querySelectorAll('[data-close-modal]').forEach(b=>b.addEventListener('click',()=>b.closest('.gm-modal').hidden=true));content.querySelectorAll('[data-action]').forEach(b=>b.addEventListener('click',()=>messageAction(b.dataset.action,content)));content.querySelectorAll('[data-compose-mode]').forEach(b=>b.addEventListener('click',()=>openCompose(b.dataset.composeMode)));content.querySelector('[data-abrir-os]')?.addEventListener('click',abrirOsFromEmail);content.querySelectorAll('[data-attachment]').forEach(b=>b.addEventListener('click',async()=>{const{data,error}=await supabase.storage.from('email-anexos').createSignedUrl(b.dataset.attachment,60);if(error)return alert(error.message);window.open(data.signedUrl,'_blank','noopener');}));content.querySelector('#gmDisconnect').addEventListener('click',async()=>{if(!confirm('Desconectar a conta e apagar todas as mensagens sincronizadas?'))return;const{data,error}=await supabase.functions.invoke('gestor-email-account',{body:{action:'disconnect'}});if(error||!data?.ok)return alert(data?.error||error?.message);state.account=null;state.messages=[];state.selected=null;renderConnect(content);});}
