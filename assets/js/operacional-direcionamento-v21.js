...
(function () {
  ...
  const STYLE_ID = 'opv21-colabs-dist-style';
  ...
  async function loadDeslocamento() {
    ...
    console.info('[opv21] deslocamento conferência', {
    ...
    for (const arr of grupos.values()) {
      ...
      for (const item of arr) {
        if (item.tipo === 'uber') {
          ...
          // Mesmo destino/ponto: não soma viagem cheia. Mantém um desvio estimado pequeno para custo operacional.
  ...
  function collect(root) {
    const items = classifyRows(getVisibleRows(root));
  ...
  function renderCards(root, totals) {
    ...
    upsertDistanceCard(grid, 'dist-carona', 'Distância carona/desvio', totals.carona);
  ...
  function renderColabs(root, colabs) {
    ...
    card.innerHTML = `
      ...
      <div class="opv15-colabs-note">
        Carona: quando existe frota no mesmo destino/ponto, colaboradores sem frota e fora da lista de Deslocamento passam para Carona/frota. Para raio real de 5 km entre pontos próximos, o próximo passo é levar essa regra para a montagem principal das rotas com coordenadas.
      </div>
      ${body}
    `;
  }
