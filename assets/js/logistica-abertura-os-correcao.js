...
import { validarLocalEmbarque, comporLocalDestino, localSemUfCidade } from './logistica-locais-servico.js?v=20260924-destino1';
...
function payload(){const opcoes=testesSelecionados();const categoria=categoriaProduto(val('osProduto'));return {contratante_cliente:val('osContratante'),filial_pagadora:val('osFilialPagadora'),produtor:val('osProdutor')||null,armazem_embarque:val('osArmazemEmbarque'),uf_embarque:val('osUfEmbarque'),cidade_embarque:val('osCidadeEmbarque'),uf_destino:val('osUfDestino'),cidade_destino:val('osCidadeDestino'),local_destino:comporLocalDestino(val('osUfDestino'),val('osCidadeDestino'),val('osLocalDestino')),numero_contrato:val('osNumeroContrato'),produto:val('osProduto'),tipo_produto:val('osTipoProduto'),volume_inicial:parseNum(val('osVolumeInicial')),regional:val('osRegional'),troca_notas:val('osTrocaNotas'),servico:val('osServico'),testes:categoria?{categoria,opcoes}:{}};}
...
function fill(row){const values={osContratante:row.contratante_cliente,osFilialPagadora:row.filial_pagadora,osProdutor:row.produtor,osUfEmbarque:row.uf_embarque,osCidadeEmbarque:row.cidade_embarque,osArmazemEmbarque:row.armazem_embarque,osUfDestino:row.uf_destino,osCidadeDestino:row.cidade_destino,osLocalDestino:localSemUfCidade(row.local_destino),osNumeroContrato:row.numero_contrato,osProduto:row.produto,osTipoProduto:row.tipo_produto,osVolumeInicial:row.volume_inicial,osRegional:row.regional,osTrocaNotas:row.troca_notas,osServico:row.servico};Object.entries(values).forEach(([id,value])=>{const input=document.getElementById(id);if(!input)return;if(input instanceof HTMLSelectElement)ensureOption(input,value??'');input.value=value??'';input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));});
