(function () {
  ...
  const STYLE_ID = 'opv16-colabs-dist-style';
  let observer = null;
  let applyTimer = null;
  let applying = false;
  ...
  function css() {
    ...
    style.textContent = `
      ...
      .opv15-colabs-table td:nth-child(6) {
        text-align: right;
      }
  ...
  function getOs(row) {
    ...
    const m = text.match(/OS\s+([^·\s]+)/i);
    return m ? m[1].trim() : '';
  }

  function getPointGroup(row) {
    // Sem coordenadas no DOM, usamos o primeiro trecho de identificação operacional disponível.
    // Isso evita multiplicar 19 OS do mesmo destino como 19 deslocamentos completos.
    const text = norm(row.textContent || '');
    const os = getOs(row);
    return text
      .replace(/OS\s+[A-Z0-9-]+/i, '')
      .replace(/SALDO.*$/i, '')
      .slice(0, 120) || os;
  }
  ...
  function collect(root) {
    ...
    for (const row of rows) {
      ...
      const pointGroup = getPointGroup(row);
      ...
      // Conta a distância uma vez por grupo/ponto, mantendo a maior estimativa do grupo.
  ...
  function renderColabs(root, colabs) {
    ...
    card.innerHTML = `
      ...
      <div class="opv15-colabs-note">
        Distância corrigida: conta uma vez por colaborador + ponto/grupo, não uma vez por OS. Para rota real exata com sequência de pontos, precisa consolidar as coordenadas por colaborador.
      </div>
      ${body}
    `;
  }
