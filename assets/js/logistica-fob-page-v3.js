...
function todayBr() { return brDate(new Date()); }
...
function rowDate(row) {
  return isoDate(pick(row, ['Data', 'Data OS', 'Data da OS', 'Data NHE', 'Última Atualização']));
}
...
async function fetchAgentToday(table, maxRows, label) {
  const dateValues = [todayBr(), todayIso()];
  ...
  if (!collected.length) {
    ...
    for (const dateValue of dateValues) {
      try {
        ...
        if (!isTimeout(error)) console.warn(`[FOB] ${label}`, error);
      }
    }
    if (!collected.length && (fallbackErrors.length || recentErrors.length)) {
      const error = fallbackErrors[0] || recentErrors[0];
      warnings.push(`${label}: não foi possível consultar a base (${error.message || error}).`);
    }
  }

  const unique = new Map();
  collected.forEach((record) => {
    const key = `${record.created_at || ''}|${JSON.stringify(record.dados_json || {})}`;
    unique.set(key, record);
  });

  const rows = [...unique.values()].sort((a, b) => String(a.created_at || '').localeCompare(String(b.created_at || '')));
...
function buildReport(mapaRecords, producaoRecords, nheRecords, operacionalRows) {
  ...
  nheRows.forEach((row) => {
    const os = normOs(pick(row, ['O.S.', 'OS', 'O.S', 'O S']));
    const date = rowDate(row) || todayIso();
    if (os) {
      nheOs.add(os);
...
function injectStyles() {
  if (document.getElementById('fob-v3-styles')) return;
  const style = document.createElement('style');
  style.id = 'fob-v3-styles';
  style.textContent = `
    .fob-actions{display:flex;gap:8px;flex-wrap:wrap}.fob-actions .btn{width:auto!important}.fob-note{border:1px solid rgba(59,130,246,.28);background:rgba(59,130,246,.08);color:#bfdbfe;border-radius:16px;padding:12px;margin-top:12px;font-size:13px}.fob-warning{border-color:rgba(245,158,11,.3);background:rgba(245,158,11,.08);color:#fde68a}.fob-grid{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:12px}.fob-diag{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.fob-diag span{border:1px solid rgba(148,163,184,.18);border-radius:999px;padding:6px 10px;font-size:12px;color:#cbd5e1;background:rgba(15,23,42,.28)}
  ...
  content.innerHTML = `
    <section class="card mt-16">
      <div class="section-head">
        <div><h3>FOB — Comparação automática</h3><p class="muted">Usa o Mapa de Embarque do dia como fonte de movimento e cruza com Produção Diária, NHE e cadastro de O.S.</p></div>
        <div class="fob-actions"><button id="fobReload" class="btn btn-secondary">↻ Atualizar</button><button id="fobSave" class="btn btn-secondary" disabled>Salvar pendentes no painel</button><button id="fobCsv" class="btn btn-secondary" disabled>Exportar CSV</button></div>
      </div>
      <div class="fob-note">Entra no FOB toda O.S. do Mapa com <strong>Tons Hoje = 0</strong>. NHE ou Produção com Cargas = NHE confirma <strong>OK</strong>; duplicidade de Cliente + Cidade + Local + Data indica <strong>DOIS EMBARQUES</strong>; o restante fica <strong>PENDENTE</strong>.</div>
      <div id="fobFeedback" class="feedback mt-16">Carregando comparação...</div>
      <div id="fobWarnings"></div>
      <div id="fobResult" class="mt-16"></div>
    </section>
    <section class="card mt-16"><h3>Imagem por Regional</h3><p class="muted">Agrupa o resultado por Supervisão e gera imagem ou ZIP.</p><div class="fob-form" style="grid-template-columns:minmax(240px,1fr) auto auto"><select id="fobRegional" class="fob-input"><option value="">Selecione</option></select><button id="fobImage" class="btn btn-primary">Gerar imagem</button><button id="fobZip" class="btn btn-secondary">Gerar ZIP (todas)</button></div><div id="fobRegionalFeedback" class="fob-note" style="display:none"></div></section>
    <details class="card mt-16 fob-subcard"><summary style="cursor:pointer;font-weight:900;color:#bbf7d0">Lançamento manual / histórico de validação</summary><div class="card mt-16"><div class="fob-form"><input id="fobData" class="fob-input" type="date" value="${todayIso()}"><input id="fobOs" class="fob-input" placeholder="Número da O.S."><input id="fobSup" class="fob-input" placeholder="Supervisão"><input id="fobCliente" class="fob-input" placeholder="Cliente"><input id="fobObs" class="fob-input" placeholder="Observação"><button id="fobManual" class="btn btn-primary">Registrar FOB 0</button></div></div><div id="fobHistory" class="mt-16"></div></details>
  `;
}
...
function renderResult() {
  ...
  if (!state.rows.length) {
    host.innerHTML = '<div class="fob-empty">Nenhuma O.S. com Tons Hoje = 0 foi localizada no Mapa de Embarque do dia.</div>';
  ...
  host.innerHTML = `
    ...
    <div class="fob-diag"><span>Mapa: ${BR_INT.format(stats.mapa || 0)} O.S.</span><span>Cadastro O.S.: ${BR_INT.format(stats.operacional || 0)}</span><span>Produção FOB: ${BR_INT.format(stats.producao || 0)}</span><span>NHE FOB: ${BR_INT.format(stats.nhe || 0)}</span></div>
...
async function generateReport() {
  ...
  if (feedback) feedback.textContent = 'Carregando Mapa, Produção, NHE e cadastro de O.S...';

  try {
    const [mapa, producao, nhe, operacional] = await Promise.all([
      fetchAgentToday('grm_mapa_embarque_importacoes', MAX_MAPA, 'Mapa de Embarque'),
      fetchAgentToday('grm_producao_diaria_importacoes', MAX_PRODUCAO, 'Produção Diária'),
      fetchAgentToday('grm_nhe_importacoes', MAX_NHE, 'NHE'),
      fetchOperationalOs(),
    ]);
    ...
    renderWarnings();
    renderResult();
    if (feedback) feedback.textContent = `Comparação concluída: ${state.rows.length} O.S., ${state.stats.pendentes} pendente(s).`;
  } catch (error) {
    console.error('[FOB comparação]', error);
    state.rows = [];
    state.stats = null;
    renderResult();
    if (feedback) feedback.textContent = `Falha ao gerar comparação FOB: ${error.message || error}.`;
  } finally {
    state.loading = false;
  }
}
...
async function savePending() {
  ...
  const payload = rows.map((row) => ({ data_referencia: row.data, numero_os: row.os, supervisao: row.supervisao || null, cliente: row.cliente || null, tons_movimento: 0, tons_producao: 0, tons_nh: 0, observacao: [row.observacao, `Comparação automática. Local: ${row.cidade || '-'} / ${row.local || '-'}`].filter(Boolean).join(' | '), status: 'PENDENTE', criado_por: state.user?.id || null }));
...
async function validate(id, status) {
  const { error } = await supabase.from('logistica_fob').update({ status, validado_por: state.user?.id || null, validado_em: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', id);
  if (error) throw error;
  await loadHistory();
}
...
function exportCsv() {
  ...
  const anchor = document.createElement('a'); anchor.href = url; anchor.download = `FOB_${todayIso()}.csv`; anchor.click(); URL.revokeObjectURL(url);
}
...
function regionalNode(regional, rows) {
  ...
  host.innerHTML = `<h2>FOB — ${esc(regional)}</h2><p>${new Date().toLocaleString('pt-BR')} · ${rows.length} O.S.</p><table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr>${['DATA','OS','CLIENTE','SUPERVISÃO','FUNCIONÁRIO','STATUS'].map((h) => `<th style="border:1px solid #ccc;padding:6px;text-align:left">${h}</th>`).join('')}</tr></thead><tbody>${rows.map((row) => `<tr><td style="border:1px solid #ddd;padding:6px">${esc(row.data_br)}</td><td style="border:1px solid #ddd;padding:6px">${esc(row.os)}</td><td style="border:1px solid #ddd;padding:6px">${esc(row.cliente)}</td><td style="border:1px solid #ddd;padding:6px">${esc(row.supervisao)}</td><td style="border:1px solid #ddd;padding:6px">${esc(row.funcionario)}</td><td style="border:1px solid #ddd;padding:6px">${esc(row.status)}</td></tr>`).join('')}</tbody></table>`;
...
async function imageRegional(all = false) {
  ...
  if (!all) {
    const node = regionalNode(selected, groups.get(selected) || []); const canvas = await window.html2canvas(node, { scale: 2, backgroundColor: '#fff' }); downloadUrl(canvas.toDataURL('image/png'), `FOB_${selected.replace(/[^a-zA-Z0-9]+/g, '_')}.png`); node.remove(); regionalFeedback('Imagem gerada.'); return;
  }
  const zip = new window.JSZip(); const nodes = [];
  for (const [regional, rows] of groups) { const node = regionalNode(regional, rows); nodes.push(node); const canvas = await window.html2canvas(node, { scale: 2, backgroundColor: '#fff' }); zip.file(`FOB_${regional.replace(/[^a-zA-Z0-9]+/g, '_')}.png`, canvas.toDataURL('image/png').split(',')[1], { base64: true }); }
  const blob = await zip.generateAsync({ type: 'blob' }); const url = URL.createObjectURL(blob); downloadUrl(url, `FOB_regionais_${todayIso()}.zip`); URL.revokeObjectURL(url); nodes.forEach((node) => node.remove()); regionalFeedback(`ZIP gerado com ${groups.size} regional(is).`);
}
