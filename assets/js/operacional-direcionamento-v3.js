...
(function () {
  ...
  const STYLE_ID = 'opv3-direcionamento-style';
  const LEAFLET_CSS = 'leaflet-css-opv3';
  const LEAFLET_JS = 'leaflet-js-opv3';
  ...
  async function sel(table, columns = '*', fn = null, limit = 5000) {
    try {
      ...
      console.warn(`[opv3] ${table}:`, e?.message || e);
      return [];
    }
  }
  ...
  function isVeiculoComLeitura(p) {
    const sit = norm(`${p.situacao || ''} ${p.status || ''}`);
    return isVeiculoPatrimonio(p) && !/INATIV|DESLIGAD|DEMITID|REMOVID|BAIXAD/.test(sit);
  }
  ...
  function distLinha(p, a, b) {
    const latKm = 111;
    const lngKm = 111 * Math.cos(((a.lat + b.lat) / 2) * Math.PI / 180);
    const ax = a.lng * lngKm, ay = a.lat * latKm, bx = b.lng * lngKm, by = b.lat * latKm, px = p.lng * lngKm, py = p.lat * latKm;
    const dx = bx - ax, dy = by - ay, l2 = dx * dx + dy * dy;
    if (!l2) return { d: Math.hypot(px - ax, py - ay), t: 0 };
    const t = Math.max(0, Math.min(1, ((px - ax) * dx + (py - ay) * dy) / l2));
    return { d: Math.hypot(px - (ax + t * dx), py - (ay + t * dy)), t };
  }

  function carona(c, p, cands) {
    if (!geo(c) || !geo(p)) return null;
    const pass = { lat: lat(c), lng: lng(c) };
    const dest = { lat: lat(p), lng: lng(p) };

    return cands
      .filter(x => x.id !== c.id && x.temFrota && geo(x))
      .map(m => {
        const origem = { lat: lat(m), lng: lng(m) };
        const linha = distLinha(pass, origem, dest);
        const direto = km(origem.lat, origem.lng, dest.lat, dest.lng) || 0;
      ...
      .filter(x => x.ok)
      .sort((a, b) => a.desvio - b.desvio || a.linha - b.linha)[0] || null;
  }
  ...
  async function load() {
    ...
    st.os = osRaw
      ...
      .map(o => {
        const p = pontoDaOs(o);
        return p ? { ...o, __pontoKey: p.__key } : null;
      })
      .filter(Boolean);

    const open = new Set(st.os.map(o => String(o.id)));
    ...
    st.vinculos = vincRaw
      .filter(v => open.has(String(v.os_id)))
      .map(v => {
        const key = colKey(v);
  ...
  function build() {
    ...
    const osBy = new Map(st.os.map(o => [String(o.id), o]));
    const all = st.vinculos.map(v => v.__colab).filter(c => c.nome);
    ...
    for (const v of st.vinculos) {
      const os = osBy.get(String(v.os_id));
      const p = pontos.get(os?.__pontoKey);
      const c = v.__colab;
      if (!os || !p || !c?.nome) continue;

      if (!geo(c)) {
        st.ignorados.push({ colaborador: c.nome, os, motivo: 'Endereço sem coordenada válida no Brasil' });
        continue;
      }

      const d = km(lat(c), lng(c), lat(p), lng(p));
      if (d == null) {
        st.ignorados.push({ colaborador: c.nome, os, motivo: 'Coordenada inválida' });
        continue;
      }

      const car = carona(c, p, all);
  ...
  function css() {
    ...
    s.textContent = `
      ...
      .opv2-row.active,.opv2-row:hover{border-color:#22c55e;background:rgba(22,101,52,.13)}.opv2-row strong{display:block;color:#fff;font-size:13px}.opv2-row small{display:block;color:#94a3b8;margin-top:4px}
      ...
      .ok{color:#bbf7d0}.warn{color:#fde68a}.bad{color:#fecaca}
      ...
      .mk{width:18px;height:18px;border-radius:50%;border:2px solid #fff}.mk.ponto{background:#22c55e}.mk.colab{background:#60a5fa}.mk.carona{background:#f59e0b}
  ...
  function rows() {
    return st.rotas.filter(r => !st.ponto || r.ponto.__key === st.ponto);
  }
  ...
  function html() {
    const k = kpis();
    const rs = rows();
    const alerta = k.ignorados
      ? `<div class="opv2-alert">${k.ignorados} colaborador(es) com endereço/coordenada fora do Brasil foram desconsiderados do mapa.</div>`
      : '';
        ...
        <section class="opv2-card">
          <div class="opv2-head">
            <div>
              ...
              <p>Somente OS abertas. Coordenadas fora do Brasil são ignoradas para evitar rotas falsas.</p>
            </div>
  ...
  function detail() {
    const r = st.rotas.find(x => x.id === st.rota) || rows()[0];
  ...
  function draw() {
    ...
    const rs = rows();
    const r = st.rotas.find(x => x.id === st.rota) || rs[0];
    const seen = new Set();
    ...
    if (r && geo(r.colab) && geo(r.ponto)) {
      const c = [lat(r.colab), lng(r.colab)];
      const p = [lat(r.ponto), lng(r.ponto)];
      const pts = r.carona && geo(r.carona.motorista)
        ? [[lat(r.carona.motorista), lng(r.carona.motorista)], c, p]
        : [c, p];

      L.polyline(pts, { color: r.carona ? '#f59e0b' : '#22c55e', weight: 4, opacity: .9, dashArray: r.colab.temFrota ? null : '7 7' }).addTo(st.layer);

      if (r.carona && geo(r.carona.motorista)) {
        L.marker(pts[0], { icon: icon('carona') })
          .bindTooltip(`Frota/carona: ${esc(r.carona.motorista.nome)}`)
          .addTo(st.layer);
      }

      L.marker(c, { icon: icon('colab') })
        .bindTooltip(`Colaborador: ${esc(r.colab.nome)}`)
        .addTo(st.layer);

      pts.forEach(x => b.push(x));
    }
  ...
  async function openHome(root) {
    ...
    st.rota = rows()[0]?.id || '';
    render(root);
    console.info('[opv3] direcionamento carregado', {
      os: st.os.length,
      rotas: st.rotas.length,
      veiculosLeitura: st.veiculosLeitura.length,
      frotaOs: st.rotas.filter(r => r.colab.temFrota).length,
