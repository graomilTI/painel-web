...
(function () {
  ...
  async function sel(table, columns = '*', fn = null, limit = 5000) {
    try {
      ...
      console.warn(`[opv2] ${table}:`, e?.message || e);
      return [];
    }
  }
  ...
  async function map(root) {
    ...
    st.layers.r = L.layerGroup().addTo(st.map);
    draw();
  ...
  function draw() {
    ...
    if (r && geo(r.colab) && geo(r.ponto)) {
      ...
      L.polyline(pts, { color: r.carona ? '#f59e0b' : '#22c55e', weight: 4, opacity: .9, dashArray: r.colab.temFrota ? null : '7 7' }).addTo(st.layers.r);
