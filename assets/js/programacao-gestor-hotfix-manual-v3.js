...
const S={poolKey:'',pool:[],poolPromise:null,poolPromiseKey:'',map:null,mapEl:null,osLayer:null,colabLayer:null,markers:[],mapKey:'',boundsKey:'',mapTimer:null};
...
#pgcPane2 #peqbAutoPreencher,#pgcPane2 #peqbSugerirCaronas,#pgcPane2 #peqbCaronasMsg,#pgcPane2 #peqbVerMapa,#pgcPane2 .peqb-legend,#pgcPane2 [data-confirmar],#pgcPane2 .peqb-cand,#pgcPane2 .peqb-cand-more,#pgcPane2 .peqb-cand-list,#pgcPane2 [data-outros],#pgcPane2 .peqb-row-btn.hotel,#pgcPane2 .peqb-cand-flag.hotel{display:none!important}#pgcPane2 .peqb-toolbar::after{content:'Vinculação manual: arraste no mapa ou na lista lateral.';font-size:11px;color:#9fb7aa;align-self:center}#pgcPane2 #peqbMapBand{display:block!important;margin:0 0 12px!important}.pmg-map{height:min(540px,calc(100vh - 320px))!important;min-height:340px!important}.pmg-drag-icon{cursor:grab!important}.leaflet-dragging .pmg-drag-icon{cursor:grabbing!important}.leaflet-marker-icon.pmg-drag-icon{pointer-events:auto!important}.pmg-help{font-size:10.5px;color:#9fb7aa}.pmg-pending{animation:pmgPending .8s ease-in-out infinite!important}@keyframes pmgPending{0%,100%{opacity:1}50%{opacity:.3}}
#pgcPane3 .prog-section-title{margin-bottom:6px!important;align-items:center!important}#pgcPane3 .prog-section-title h4{font-size:18px!important;letter-spacing:0!important}.pgc-desp-head,#pgcPane3 .peqd-card{display:grid!important;grid-template-columns:minmax(170px,1.05fr) minmax(315px,1.8fr) minmax(170px,.82fr) minmax(330px,1.55fr) minmax(240px,1fr);gap:0}.pgc-desp-head{border:1px solid rgba(52,211,153,.18);border-radius:10px 10px 0 0;overflow:hidden;background:rgba(2,6,23,.72);color:#d9fbe8;font-size:11px;font-weight:950;text-transform:uppercase;box-shadow:inset 0 -1px 0 rgba(148,163,184,.10)}.pgc-desp-head span{padding:10px 8px;text-align:center;border-right:1px solid rgba(52,211,153,.14)}#pgcPane3 .peqd-list{gap:0!important;overflow:auto;border:1px solid rgba(52,211,153,.16);border-top:0;border-radius:0 0 10px 10px}#pgcPane3 .peqd-card{align-items:center;padding:0!important;border-radius:0!important;border:0!important;border-top:1px solid rgba(148,163,184,.10)!important;background:rgba(2,6,23,.18)!important;min-width:1225px}#pgcPane3 .peqd-head,#pgcPane3 .peqd-sec{margin:0!important;padding:6px 8px!important;border-top:0!important;border-right:1px solid rgba(148,163,184,.10);min-height:38px;box-sizing:border-box;min-width:0}#pgcPane3 .peqd-sec-label{display:none!important}#pgcPane3 .peqd-head{align-items:center!important;margin-bottom:0!important;overflow:hidden}#pgcPane3 .peqd-av{width:26px!important;height:26px!important}#pgcPane3 .peqd-nome{font-size:12px!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#pgcPane3 .peqd-os-ref{font-size:10px!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#pgcPane3 .peqd-row,#pgcPane3 .peqd-chips{gap:5px!important;flex-wrap:nowrap!important}#pgcPane3 .peqd-inp{min-height:29px!important;font-size:11px!important;padding:4px 6px!important}#pgcPane3 .peqd-obs,#pgcPane3 .peqd-extra-obs{display:none!important}#pgcPane3 [data-sec="extras"]{display:flex!important;flex-wrap:wrap!important;align-items:center!important;gap:7px!important}#pgcPane3 .peqd-extra-list{flex:1 1 220px!important;min-width:0!important;overflow:hidden!important}#pgcPane3 .peqd-extra-item{display:grid!important;grid-template-columns:minmax(74px,1fr) 72px 28px!important;gap:5px!important;align-items:center!important;padding:0!important;border:0!important;background:transparent!important;min-width:0!important}#pgcPane3 .peqd-extra-desc{display:none!important}#pgcPane3 .peqd-extra-tipo,#pgcPane3 .peqd-extra-valor{min-width:0!important;width:100%!important}#pgcPane3 .peqd-extra-rm{width:28px!important;height:29px!important}#pgcPane3 .peqd-extra-add{flex:0 0 150px!important;width:150px!important;max-width:150px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;margin-top:0!important}.pgc-hotel-cidade{flex:1 1 130px!important;min-width:110px!important}
@media(max-width:1200px){.pgc-desp-head{display:none!important}#pgcPane3 .peqd-card{grid-template-columns:1fr!important;border-radius:12px!important;border:1px solid rgba(52,211,153,.18)!important;margin-bottom:8px;min-width:0!important}}@media(max-width:980px){.pgc-os-kpi-head{display:none!important}#pgcPane1 .peqs-row .peqb-os2-left{grid-template-columns:1fr 1fr!important}}@media(max-width:720px){#pgcPane1 .peqs-row .peqb-os2-left{display:block!important}#pgcPane1 .peqs-row .peqb-os2-kpi,#pgcPane1 .peqs-row .peqb-os2-tagsrow{margin-bottom:7px!important}}
`;document.head.appendChild(s)}
function step1(){const pane=document.getElementById('pgcPane1'),list=pane?.querySelector('#peqsOsList,.peqb-os-list');if(!pane||!list)return;if(!pane.querySelector('.pgc-os-kpi-head'))list.insertAdjacentHTML('beforebegin','<div class="pgc-os-kpi-head"><span>Cliente</span><span>Local de embarque</span><span>Rem.</span><span>O.S.</span><span>Ações</span></div>');pane.querySelectorAll('.peqb-os2-kpi strong').forEach(s=>{s.textContent=String(s.textContent||'').replace(/\s+/g,' ').trim()})}
function despesas(){const pane=document.getElementById('pgcPane3'),list=pane?.querySelector('#peqdRoster');if(!pane||!list)return;if(!pane.querySelector('.pgc-desp-head'))list.insertAdjacentHTML('beforebegin','<div class="pgc-desp-head"><span>Colaborador</span><span>Estadia</span><span>Alimentação</span><span>Deslocamento</span><span>Extras</span></div>');pane.querySelectorAll('.peqd-card').forEach(card=>{if(norm(card.querySelector('[data-fld="tipo_estadia"]')?.value||'')!=='HOTEL')return;const dest=card.querySelector('[data-estadia-destino]');if(!dest)return;const cid=cidadeEmb(card.dataset.embarque)||card.querySelector('[data-fld="cidade"]')?.value||'';dest.innerHTML=`<input class="peqd-inp peqd-inp-sm pgc-hotel-cidade" data-tab="estadia" data-fld="cidade" value="${esc(cid)}" placeholder="Cidade do embarque" />`})}
function addPool(m,row,extras={}){
  const id=String(row?.colaboradorId||row?.colaborador_id||cpf(row?.cpf)||row?.id||'').trim();
...
window.__pmgRenderMapaGestor=()=>mapRender({force:true});
...
async function vincOs(osId,col){const{snap,item}=findOs(osId);if(!snap||!item)return;const cand=mat(item,col);if((item.equipeRows||[]).some(r=>String(r.colaborador_id)===String(cand.colaboradorId))){alert(`${cand.nome} já está vinculado nesta O.S.`);return}const atuais=item.equipeRows||[];const disponibilidade=isMot(cand)?await dispMot(cand.nome):'OK';if(!disponibilidade)return;let motivo=null;if(disponibilidade==='OK'&&atuais.length){motivo=await just(item.os,atuais,cand);if(!motivo)return}const programacaoId=snap.programacaoIdParaOs?.(item.os)||item.equipeRows?.[0]?.programacao_id;if(!programacaoId){alert('Programação da O.S. não encontrada. Recarregue a tela.');return}await upsert({programacaoId,os:item.os,cand,disponibilidade});if(motivo)logActivity('action','justificativa_multiplos_colaboradores_os','programacao',{os_id:item.os.id,numero_os:item.os.numero_os,colaboradores:[...atuais.map(c=>c.colaborador_id),cand.colaboradorId],nomes:[...atuais.map(c=>c.nome_colaborador),cand.nome],motivo});afterWrite()}
async function vincMotCol(osId,alvoId,motorista){const{snap,item}=findOs(osId);if(!snap||!item)return;if(!alvoId)return vincOs(osId,motorista);const programacaoId=snap.programacaoIdParaOs?.(item.os)||item.equipeRows?.[0]?.programacao_id;if(!programacaoId){alert('Programação da O.S. não encontrada. Recarregue a tela.');return}const disponibilidade='LOGISTICA';const cand=mat(item,motorista);await upsert({programacaoId,os:item.os,cand,disponibilidade});const pl=placa(cand.veiculoPlaca||'');if(pl)await supabase.from('programacao_deslocamento').upsert({programacao_id:programacaoId,data_referencia:snap.dataReferencia||hoje(),colaborador_id:alvoId,tipo_deslocamento:'CARONA FROTA',placa_veiculo:pl,observacao:`Motorista vinculado na programação: ${cand.nome}`},{onConflict:'programacao_id,colaborador_id'});logActivity('action','motorista_vinculado_colaborador_os','programacao',{os_id:item.os.id,numero_os:item.os.numero_os,motorista_id:cand.colaboradorId,motorista_nome:cand.nome,colaborador_id:alvoId,placa:pl,disponibilidade});afterWrite()}
async function associar(a,b){const os=[a,b].find(x=>x.tipo==='os'),p=[a,b].find(x=>x.tipo==='colaborador'||x.tipo==='motorista');if(os&&p?.colab)return vincOs(os.osId,p.colab);const mot=[a,b].find(x=>x.tipo==='motorista'),col=[a,b].find(x=>x.tipo==='colaborador'&&x.osId);if(mot?.colab&&col?.colaboradorId&&col?.osId)return vincMotCol(col.osId,col.colaboradorId,mot.colab);alert('Solte o marcador sobre uma O.S. ou sobre um colaborador já vinculado à O.S.')}
