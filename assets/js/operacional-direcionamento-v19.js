...
(function () {
  ...
  const STYLE_ID = 'opv19-colabs-dist-style';
  ...
  async function loadDeslocamento() {
    ...
    console.info('[opv19] deslocamento conferência carregado', {
  ...
  function colaboradorEmDeslocamento(nome) {
    return deslocamentoNomes.has(norm(nome));
  }
  ...
  function getTipo(row) {
    ...
    // Particular só se estiver listado no Deslocamento do menu Conferência.
    if (colaboradorEmDeslocamento(nome)) return 'particular';

    // Sem frota e sem registro em deslocamento não pode virar veículo próprio.
    return 'uber';
  }
  ...
  function collect(root) {
    ...
    for (const row of rows) {
      ...
      const pointGroup = norm(getDestino(row));
      ...
      const g = current.grupos.get(pointGroup) || { km: 0, os: 0, saldo: 0 };
      g.km = Math.max(g.km, km);
      g.os += 1;
      g.saldo += Number(saldoOs || 0);
      current.grupos.set(pointGroup, g);

      colabs.set(key, current);
    }

    const normalized = [...colabs.values()].map(c => {
      c.km = [...c.grupos.values()].reduce((acc, g) => acc + g.km, 0);
      c.pontos = c.grupos.size;
      totals[c.tipo] += c.km;
      return c;
  ...
  function renderCards(root, totals) {
    ...
    upsertDistanceCard(grid, 'dist-uber', 'Distância total Uber/A definir', totals.uber);
  }
  ...
  function renderColabs(root, colabs) {
    ...
    const totalPontos = colabs.reduce((acc, c) => acc + c.pontos, 0);
    ...
    card.innerHTML = `
      <div class="opv15-colabs-head">
        ...
        <span>${colabs.length} colaborador(es) · ${totalOs} OS · ${totalPontos} ponto(s)/grupo(s)</span>
      </div>
      <div class="opv15-colabs-note">
        Veículo próprio/Particular só é considerado para colaboradores listados em Deslocamento do menu Conferência. Demais sem frota ficam como Uber/A definir.
      </div>
      ${body}
    `;
  }
