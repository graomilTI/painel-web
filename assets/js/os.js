...
initProtectedPage('OS', async (content) => {
  ...
  function sugestoesParaOs(row) {
    ...
    const cols = state.colaboradores.filter((c) => !supKey || normalize(c.supervisao || c.regional).includes(supKey) || supKey.includes(normalize(c.supervisao || c.regional)));
    return cols.map((c) => {
      const dist = haversineKm(row.ponto1_latitude, row.ponto1_longitude, c.latitude, c.longitude);
      return { ...c, distancia_km: dist };
    }).sort((a, b) => {
      const ad = a.distancia_km ?? 999999;
      const bd = b.distancia_km ?? 999999;
  ...
  function render() {
    ...
    el.list.innerHTML = `<div class="os-table-wrap"><table class="os-table"><thead><tr><th>O.S.</th><th>Cliente / rota</th><th>Remanescente</th><th>Sugestão operacional</th><th>Indicação</th><th>Ação gestor</th></tr></thead><tbody>${rows.map(rowHtml).join('')}</tbody></table></div>`;
  }
  ...
  function rowHtml(row) {
    ...
    return `<tr data-os-id="${escapeHtml(row.id)}" class="${zero ? 'os-zero' : ''}">
      <td><div class="os-title">O.S. ${escapeHtml(row.numero_os)}</div><div class="os-meta">${brDate(row.data_os)} • ${escapeHtml(first(row.servico))}</div><div class="os-meta">${escapeHtml(first(row.supervisao))}</div>${zero ? '<div class="os-warn-text">Remanescente zerado</div>' : ''}</td>
      ...
      <td>${principal ? `<div class="os-sug"><strong>${escapeHtml(principal.nome || principal.nome_colaborador || 'Colaborador')}</strong><div class="muted">${principal.distancia_km == null ? 'Sem coordenada para calcular distância' : `${KM.format(principal.distancia_km)} km do ponto 1`}</div><div class="os-mini">Primeiro da listagem operacional por menor distância.</div></div>` : '<span class="muted">Sem colaborador com base operacional localizada.</span>'}</td>
      <td>
  ...
  async function onListClick(event) {
    ...
    if (statusBtn) {
      ...
      await updateOs(tr.dataset.osId, { status_gestor: statusBtn.dataset.status, configurada_em: new Date().toISOString() });
      return;
    }
