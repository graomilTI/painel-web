...
function injectStyles() {
  if (document.getElementById('fob-v4-styles')) return;
  const style = document.createElement('style');
  style.id = 'fob-v4-styles';
  style.textContent = `
  ...
  content.innerHTML = `
    <section class="card mt-16">
      <div class="section-head">
        <div><h3>FOB — Comparação automática</h3><p class="muted">Analisa exclusivamente o Mapa de Embarque do dia anterior e cruza com Produção Diária, NHE e cadastro de O.S.</p></div>
...
function renderResult() {
  ...
  host.innerHTML = `
    ...
    <div class="fob-diag"><span>Referência: ${referenceBr()}</span><span>Mapa: ${BR_INT.format(stats.mapa || 0)} O.S.</span><span>Cadastro O.S.: ${BR_INT.format(stats.operacional || 0)}</span><span>Produção FOB: ${BR_INT.format(stats.producao || 0)}</span><span>NHE FOB: ${BR_INT.format(stats.nhe || 0)}</span></div>
...
async function generateReport() {
  ...
  if (feedback) feedback.textContent = `Carregando Mapa, Produção e NHE de ${referenceBr()}...`;
  ...
  try {
    const [mapa, producao, nhe, operacional] = await Promise.all([
      ...
      fetchAgentReferenceDay('grm_producao_diaria_importacoes', MAX_PRODUCAO, 'Produção Diária'),
