...
marker = "async function carregarJaLancadas(dataReferencia) {"
guard = """var nheRealPorChaveCache = {};
...
replace_once(marker, guard, 'existeNheReal')
...
old_loop = """    for (var i = 0; i < pendentes.length; i++) {
      var p = pendentes[i];
      if (jaLancadas[chaveUnica(p.data, p.os)]) continue;

      if (!p.funcionario) {"""
new_loop = """    for (var i = 0; i < pendentes.length; i++) {
      var p = pendentes[i];
      ...
      if (!p.funcionario) {"""
replace_once(old_loop, new_loop, 'trava no loop')

old_cont = """    var restantes = Math.max(0, totalCandidatos - candidatos.length);
...
new_cont = """    var restantes = Math.max(0, totalCandidatos - candidatos.length);
...
replace_once(old_cont, new_cont, 'controle continuação')

replace_once(
    "  buscarGestorRegional: buscarGestorRegional\n};",
    "  buscarGestorRegional: buscarGestorRegional,\n  existeNheReal: existeNheReal\n};",
    'export existeNheReal',
)

path.write_text(text, encoding='utf-8')
print('Patch NHE aplicado com sucesso.')
